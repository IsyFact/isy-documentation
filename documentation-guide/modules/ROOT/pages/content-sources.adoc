...
.Konfiguration der Content Source für die IsyFact-Standards (Release 3.x) in 'antora-playbook.yml'
...
* der https://docs.antora.org/antora/latest/playbook/content-tags/[tag key] selektiert eine Reihe von Git-Tags.
* das Version-Mapping legt den version key für eine Antora-Komponente fest.

NOTE: Beinhaltet ein Content-Source mehrere Antora Komponenten, so ist der
https://docs.antora.org/antora/latest/playbook/content-source-start-paths/#start-paths-key[_start_paths_] key zu verwenden.
...
Für die lokale Entwicklung ist es sinnvoll ein weiteres Playbook _antora-playbook-local.yml_ zu pflegen.
...
.Konfiguration der Content Source für die IsyFact-Standards (Release 3.x) in 'antora-playbook-local.yml'
...
content:
  sources:
    - url: ../isyfact-standards
      branches: master, feature/IFS-4549*, release/4*, release/3.x*, release/3.2.1
      tags: [4.0.0]
      start_path: isyfact-standards-doc/src/docs/antora
      version:
        'feature/(*)': $1
...
WARNING: Antora rät generell davon ab, Dokumentation aus Tags zu ziehen, siehe https://docs.antora.org/antora/latest/playbook/content-tags/[Content Keys - Tags]

NOTE: Das lokale checkout der Dokumentation mit Git LFS kann über eine Antora Erweiterung realisiert werden, siehe https://docs.antora.org/antora/latest/extend/extension-use-cases/#support-remote-repositories-with-git-lfs-enabled[Antora extension Use Cases]