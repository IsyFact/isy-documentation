...
NOTE: Für diesen Guide ist ein grundlegendes Verständnis der https://docs.antora.org/antora/latest/component-version-descriptor/[Antora-Komponenten-Konfigurationsdatei `antora.yml`] und der https://docs.antora.org/antora/latest/playbook/configure-content-sources/[Antora-Playbook-Konfiguration] nützlich.
...
Referenznamen-Projektion ermöglicht es, den Referenznamen mit einem Muster abzugleichen und dann eine Version auf der Grundlage dieser Übereinstimmung zu erstellen.
...
* (main|master|develop): Entwicklungsbranches (main, master oder develop) wird die Version `NEXT` zugewiesen.
* release/(*): Releasebranches (beispielsweise release/3.x) werden mit der Versionsnummer des Releases versehen.
* (*): Für alle anderen Branches, wie beispielsweise feature Branches wird der Branchname als Versionsnummern verwendet.

Das Mapping von Referenznamen auf Versionen kann sowohl für die Antora-Komponente in antora.yml, als auch im Antora Playbook für eine Content-Source definiert werden.
...
Für Entwicklungsstände gibt es eine weitere Möglichkeit die angezeigten Komponentenversionen anzupassen.
