...
[TIP]
====
Die Online-Dokumentation der IsyFact nutzt als Konvention den Ordner `docs` auf oberster Ebene eines Repositories zur Ablage einer Antora-Komponente.
...
[TIP]
====
Die Online-Dokumentation der IsyFact nutzt als Konvention den Namen des Bausteins in den Attributen `name` und `title`.

[source,yaml]
----
name: security
title: Baustein Security
----

Der Name wiederholt insbesondere nicht die Projektbezeichnung (`isy` / `isyfact`) und verweist nicht explizit darauf, das Dokumentation (u.a. `docs` / `doku`) enthalten ist.
====
