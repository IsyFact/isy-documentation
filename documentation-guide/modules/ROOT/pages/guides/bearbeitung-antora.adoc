...
Antora bietet die Möglichkeit, innerhalb einer Gesamtdokumentation beliebige Querverweise zu setzen, auch über xref:guides/erstellung-antora.adoc#antora-modul[Antora-Modul] - und xref:guides/erstellung-antora.adoc#antora-komponente[Antora-Komponenten] - Grenzen hinweg.
...
Besonderheiten ergeben sich für Anchor für <<glossar,Glossareinträge>>.
...
Seiten besitzen einen xref:seitenvorlage.adoc#struktur-thema[engen, thematischen Fokus] und sind direkt über die xref:guides/erstellung-antora.adoc#linksnavigation-antora[Linksnavigation] aufrufbar.
...
IMPORTANT: Abbildungen zu einer Seite werden stets im Unterordner `/images` unterhalb des Antora-Moduls abgelegt, in dem sich die Seite befindet, siehe auch xref:guides/erstellung-antora.adoc#struktur-dokuseite[Struktur Dokumentation].

IMPORTANT: Abbildungen sollten mit diagrams.net als <Name Abbildung>.dn.svg erstellt werden, siehe auch xref:isyfact-standards-doku:methodik:diagrammerstellung/diagramsnet.adoc#arbeiten-mit-diagrams.net[Arbeiten mit diagrams.net].
