...
Soll eine neue Dokumentation angelegt werden oder die bestehende Dokumentation um eine Seite erweitert werden, so muss zunächst das xref:index.adoc#struktur-dokumentation[geeignete Repository zur Ablage] bestimmt und ggf. lokal geklont werden.
...
Unterhalb dieses Pfades liegt die Antora-Komponente, repräsentiert durch die Konfigurationsdatei `antora.yml`.
...
Im gleichen Verzeichnis wie `antora.yml`, `src/docs/antora`, muss das https://docs.antora.org/antora/latest/standard-directories/[Verzeichnis für die Module] liegen.
...
Ein Beispiel für die Aufteilung von Inhalten auf Module findet sich in der Beschreibung der xref:index.adoc#struktur-dokumentation[Struktur der Quellen].
...
Neben dem Verzeichnis `pages` kann es bei Bedarf https://docs.antora.org/antora/latest/standard-directories/[noch weitere Verzeichnisse], z.B. für Bilder oder Anhänge, geben.
...
[[listing-nav-adoc]]
.Beispiel: Navigationsdatei des Leitfadens
[source,asciidoc]
----
include::../../nav.adoc[]
----
