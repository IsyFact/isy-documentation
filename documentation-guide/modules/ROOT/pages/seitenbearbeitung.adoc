...
Seiten müssen bestimmten Vorgaben genügen, damit sie in der Online-Dokumentation der IsyFact insgesamt in einem einheitlichen Stil zur Verfügung gestellt werden können.
Außerdem zielen die Vorgaben darauf ab, die Seiten möglichst wartungsarm zu gestalten.

Es gibt eine xref:seitenvorlage.adoc[Seitenvorlage], die kopiert werden kann, und exemplarisch alle gängigen Elemente einer Seite der Online-Dokumentation enthält.

[NOTE]
====
Der Leitfaden erklärt die Grundlagen der AsciiDoc-Syntax nicht weiter.
...
Diagramme, die mit xref:isyfact-standards-doku:methodik:diagrammerstellung/drawio.adoc[draw.io] erstellt werden, sind nach folgender Konvention zu benennen.

// tag::namenskonvention[]

[[table-namenskonvention-bilder]]
.Konvention für Dateinamen von Abbildungen aus draw.io
[cols="1s,4m",options="header"]
|====
2+|Dateinamen für Abbildungen aus draw.io
|Schema |<name-der-abbildung>.dn.svg
|Beispiel |beispiel.dn.svg
|====

// end::namenskonvention[]

Da die Online-Dokumentation der IsyFact keinen Dark Mode unterstützt, müssen alle Diagramme xref:isyfact-standards-doku:methodik:diagrammerstellung/drawio.adoc#adaptive-colors[ohne Unterstützung von adaptiven Farben] erstellt werden.
