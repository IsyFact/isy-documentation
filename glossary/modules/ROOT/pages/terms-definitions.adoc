...
Dabei kann sie die Services von <<glossar-querschnittsanwendung,Querschnittsanwendungen>> oder andere <<glossar-anwendungssystem,Anwendungssysteme>> nutzen.
|===
...
[[glossar-isyfact]]
=== IsyFact (IF)
Allgemeine Software-Fabrik (englisch: software factory) für den Bau von komplexen <<glossar-anwendungslandschaft,Anwendungslandschaften>>, die vom Bundesverwaltungsamt entwickelt wird.
...
Umfasst alle Bestandteile der IsyFact, die *nicht* für jede <<glossar-anwendung>> verpflichtend sind.
IsyFact-Erweiterungen können auch von <<glossar-nutzer,Nutzern>> der IsyFact eingebracht werden.
...
Eine vollkonforme Änderung ist eine Änderung, die das Außenverhalten einer <<glossar-bibliothek>> nicht verändert (siehe auch konform und nicht-konform).
