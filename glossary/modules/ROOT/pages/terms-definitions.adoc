...
Dabei kann sie xref:glossary::terms-definitions.adoc#service[Services] von anderen Anwendungen nutzen.
...
Eine mittels der IsyFact umgesetzte Anwendungslandschaft besteht aus <<glossar-domaene,Domänen>>, einer <<glossar-querschnittsdomaene>>, einem <<glossar-portal>> und einem <<api-gateway>>.
...
[[api-gateway-fachliche-sicht]]
==== Fachliche Sicht

// tag::api-gateway-definition-fachlich[]

Das API-Gateway ist der zentrale Zugangspunkt zum Aufruf von Schnittstellen von xref:glossary::terms-definitions.adoc#glossar-anwendung[Anwendungen] einer xref:glossary::terms-definitions.adoc#glossar-anwendungslandschaft[Anwendungslandschaft] von außen, sowie zum Aufruf von Schnittstellen von externen Anwendungen von innerhalb der Anwendungslandschaft.
Es kapselt interne Schnittstellen und bietet nach außen konsolidierte, versionierte und gesicherte APIs an.
Außerdem schützt das API-Gateway die internen Anwendungen vor Angriffen, Überlastung und unzulässigen Zugriffen, und steuert den Zugriff auf Geschäftsobjekte und ‑prozesse über fachliche Zugriffsregeln.

// end::api-gateway-definition-fachlich[]

[[api-gateway-technische-sicht]]
==== Technische Sicht

// tag::api-gateway-definition-technisch[]

Das API-Gateway ist der zentrale Zugangspunkt zum Aufruf von xref:glossary::terms-definitions.adoc#service[Services] von xref:glossary::terms-definitions.adoc#glossar-it-system[IT-Systemen] einer xref:glossary::terms-definitions.adoc#glossar-systemlandschaft[Systemlandschaft] von außen, sowie zum Aufruf von Services von xref:glossary::terms-definitions.adoc#externes-system[externen Systemen] von innerhalb der Systemlandschaft.
Es stellt eine klare technische Grenze zur Systemlandschaft dar, indem es die internen Systeme von externen Systemen entkoppelt.
Es überbrückt dabei Schnittstellentechnologien und nimmt querschnittliche Aufgaben wahr, insbesondere in den Bereichen Authentifizierung, Autorisierung, Monitoring und Protokollierung.

// end::api-gateway-definition-technisch[]
...
Ein Backend ist ein xref:glossary::terms-definitions.adoc#glossar-it-systemtyp[IT-Systemtyp], der hauptsächlich xref:glossary::terms-definitions.adoc#glossar-geschaeftslogik[Geschäftslogik] umsetzt und diese in Form von xref:glossary::terms-definitions.adoc#service[Services] bereitstellt.
...
|Ein Backend ist ein <<glossar-it-systemtyp>>, der hauptsächlich <<glossar-geschaeftslogik>> umsetzt und diese in Form von <<service,Services>> bereitstellt.
...
Eine Domäne im Kontext der <<glossar-a-architektur>> entspricht der Begrifflichkeit aus Sicht der Methode "Domain Driven Design".
...
Von externen Systemen sind nur die <<service,Services>> bekannt, die zur Kommunikation mit <<glossar-it-system,IT-Systemen>> innerhalb der betrachteten Systemlandschaft dienen.
Die Kommunikation mit externen Systemen erfolgt ausschließlich über <<gateway,Gateways>>.
...
Dabei kann sie die <<service,Services>> von <<glossar-querschnittsanwendung,Querschnittsanwendungen>> oder anderen <<glossar-anwendungssystem,Anwendungssystemen>> nutzen.
|===
...
Querschnittsanwendungen dürfen nur xref:glossary::terms-definitions.adoc#service[Services] von anderen Querschnittsanwendungen nutzen.
...
|Ein Service kapselt eine klar umrissene fachliche oder technische Funktionalität und stellt diese über eine wohldefinierte, veröffentlichte Schnittstelle bereit.
Services sind ein Bestandteil von <<backend,Backends>>.

Ein Service besitzt die folgenden Eigenschaften:

* *Modularität:* Services sind in sich abgeschlossen.
* *Definierte Schnittstelle:*  Für die Nutzung von Services reicht es, deren Schnittstellen (API) zu kennen.
Kenntnisse über die Details der Implementierung sind nicht erforderlich.
* *Autonomie:* Services verwalten ihre Daten selbst.
* *Lose Kopplung:* Services sind möglichst unabhängig voneinander realisiert.
* *Plattformunabhängigkeit:* Anbieter und Nutzer eines Services können mit unterschiedlichen Technologien auf verschiedenen Plattformen realisiert sein.

Für Services, die Funktionalität einer <<glossar-querschnittsanwendung>> umsetzen, gelten zusätzlich folgende Eigenschaften:

* *Wiederverwendbarkeit:* Services sind so definiert, dass sie von <<glossar-it-system,IT-Systemen>> verschiedener <<glossar-anwendung,Anwendungen>> bzw. <<glossar-domaene,Domänen>> verwendet werden können.
...
// tag::service-gateway-definition[]

Ein Service-Gateway ist ein xref:glossary::terms-definitions.adoc#glossar-it-system[IT-System] vom Typ xref:glossary::terms-definitions.adoc#gateway[Gateway], das eine Verbindung zwischen Services xref:glossary::terms-definitions.adoc#externes-system[externer Systeme] und xref:glossary::terms-definitions.adoc#glossar-it-system[IT-Systemen] der xref:glossary::terms-definitions.adoc#glossar-systemlandschaft[Systemlandschaft], sowie zwischen Services von xref:glossary::terms-definitions.adoc#backend[Backends] und xref:glossary::terms-definitions.adoc#externes-system[externen Systemen] herstellt.

// end::service-gateway-definition[]

Service-Gateways sind ein Teil der technischen Umsetzung des <<api-gateway,API-Gateways>>.
