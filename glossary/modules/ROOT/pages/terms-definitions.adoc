...
Sie besteht aus xref:isyfact-standards-doku:referenzarchitektur:fachlich.adoc#awl-fachliche-architektur[Anwendungskomponenten].
...
Dabei kann sie <<glossar-service-fachlich,Services>> von anderen Anwendungen nutzen.
...
Eine Anwendungslandschaft enthält <<glossar-domaene,Anwendungsdomänen>>, die wiederum Anwendungen gruppieren.
Zwischen den in einer Anwendungslandschaft vorhandenen Anwendungen ist in der Regel eine hohe Interaktion zu erwarten.

Eine mittels der IsyFact umgesetzte Anwendungslandschaft besteht aus <<glossar-domaene,Anwendungsdomänen>>, einer <<glossar-querschnittsdomaene>>, einem <<glossar-portal>> und einem <<glossar-service-gateway>>.
...
Es besteht aus xref:isyfact-standards-doku:referenzarchitektur:fachlich.adoc#awl-fachliche-architektur[Anwendungskomponenten].
...
Im Kontext der Auslieferung von sowohl Open-Source-Releases als auch Closed-Source-Releases wird Software (Bibliotheken sowie <<glossar-anwendung,Anwendungen>>) und die dazugehörige Dokumentation ausgeliefert.
...
Ein Backend ist ein xref:glossary::terms-definitions.adoc#glossar-it-systemtyp[IT-Systemtyp], der hauptsächlich xref:glossary::terms-definitions.adoc#glossar-geschaeftslogik[Geschäftslogik] umsetzt und diese in Form von <<glossar-service-fachlich,Services>> bereitstellt.
...
Backends setzen, auf die fachliche xref:glossary::terms-definitions.adoc#glossar-referenzarchitektur[Referenzarchitektur] bezogen, maßgeblich xref:isyfact-standards-doku:referenzarchitektur:fachlich.adoc#awl-fachliche-architektur[Anwendungskomponenten] und Nachbarsystemschnittstellen um.
...
Batches können hierfür entweder direkt den xref:isyfact-standards-doku:referenzarchitektur:software-technisch:backend:anwendungskern.adoc[Anwendungskern] aufrufen und nutzen, oder über interne Schnittstellen mit xref:glossary::terms-definitions.adoc#backend[Backends] kommunizieren.
...
Die Batches können dabei den xref:isyfact-standards-doku:referenzarchitektur:software-technisch:backend:anwendungskern.adoc[Anwendungskern] aufrufen und nutzen.
|===
...
Pro Bibliothek und <<glossar-anwendung>> existiert ein Change Log.
...
Eine *Anwendungsdomäne* gruppiert fachlich zusammengehörende <<glossar-geschaeftsanwendung,Geschäftsanwendungen>>.
...
Eine Geschäftsanwendung ist eine xref:glossary::terms-definitions.adoc#glossar-anwendung[Anwendung], welche einen oder mehrere Geschäftsprozesse einer xref:glossary::terms-definitions.adoc#glossar-domaene[Anwendungsdomäne] umsetzt.
...
|Eine Geschäftsanwendung ist ein <<glossar-anwendungssystem>>, welches einen oder mehrere Geschäftsprozesse einer <<glossar-domaene,Anwendungsdomäne>> spezifiziert.
...
Querschnittsanwendungen sind somit in verschiedenen xref:glossary::terms-definitions.adoc#glossar-domaene[Anwendungsdomänen] oder über mehrere xref:glossary::terms-definitions.adoc#glossar-anwendungslandschaft[Anwendungslandschaften] hinweg wiederverwendbar.

Querschnittsanwendungen dürfen nur <<glossar-service-fachlich,Services>> von anderen Querschnittsanwendungen nutzen.
...
Querschnittsanwendungen sind somit in verschiedenen <<glossar-domaene,Anwendungsdomänen>> oder über mehrere <<glossar-anwendungslandschaft,Anwendungslandschaften>> hinweg wiederverwendbar.
...
Die Herausforderung bei der Wiederverwendung besteht darin, das einmal erworbene Wissen über die Anwendungsentwicklung in einer <<glossar-domaene,Anwendungsdomäne>> so zu strukturieren, zu dokumentieren und vorzuhalten, dass nachfolgende Projekte einfach und verlässlich darauf zugreifen können, und damit die Einhaltung des Architekturrahmens sichergestellt ist.
...
Eine vollkonforme Änderung ist eine Änderung, die das Außenverhalten einer Bibliothek nicht verändert (siehe auch <<glossar-konform>> und <<glossar-nicht-konform>>).
