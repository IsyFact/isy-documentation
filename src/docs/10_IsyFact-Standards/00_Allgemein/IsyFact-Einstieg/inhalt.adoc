...
Fachliche Architektur (A-Architektur), Technische Architektur (T-Architektur) und Technische Infrastruktur (TI-Architektur).
...
=== Vorgaben für die A-Architektur
Die allgemeine Referenzarchitektur muss für die zu bauenden Systeme in einer fachlichen Architektur konkretisiert werden.
...
Ein IT-System basiert demnach auf einer Drei-Schichten-Architektur und besteht aus den Komponenten, die in <<image-image008>> dargestellt sind.
...
Datenzugriff::
Der Datenzugriff erfolgt über JPA bzw. Hibernate.
Die genaue Verwendung von JPA/Hibernate und die Prinzipien, nach denen bei der Persistierung von Datenobjekten vorgegangen werden soll, sind im Dokument <<DetailkonzeptKomponenteDatenzugriff>> festgelegt.
...
Grundlage ist die <<IsyFactReferenzarchitektur>>, welche die wesentlichen Inhalte auf grober Ebene festlegt.
...
==== Vorgaben für das Systemhandbuch
Das Systemhandbuch ist gemäß der <<IsyFactVorlageSystemhandbuch>> zu erstellen und beschreibt die Installation, Konfiguration und den Betrieb des IT-Systems.
...
(((Baustein,Überwachung und Konfiguration)))
