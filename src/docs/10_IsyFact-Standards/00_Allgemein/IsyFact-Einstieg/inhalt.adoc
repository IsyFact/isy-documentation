...
Einhaltung der <<IsyFactNamenskonventionen>> zu achten.
...
Die Realisierung hat gemäß den <<JavaProgrammierkonventionen>> zu erfolgen. Die Versionierung von Bibliotheken und Anwendungen muss sich nach den Vorgaben zur <<IsyFactVersionierung>> richten, die im Wesentlichen auf https://semver.org[Semantic Versioning] basieren. Für die Entwicklungsumgebung gelten wenige Vorgaben, die in <<LeitfadenEntwicklungsumgebung>> und <<EinrichtungEntwicklungsumgebung>> zusammengefasst sind.
