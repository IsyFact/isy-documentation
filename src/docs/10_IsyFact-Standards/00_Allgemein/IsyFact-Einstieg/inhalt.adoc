...
[[administrative-ueberwachung]]
==== Administrative Überwachung
(((Baustein,Überwachung)))
Das Dokument <<KonzeptUeberwachung>> beschreibt, wie die Überwachung und Administration einer Anwendung seitens des Systembetriebs erfolgt und welche Schnittstellen dazu durch die Anwendung zur Verfügung gestellt werden müssen.

[[administrative-konfiguration]]
==== Administrative Konfiguration
(((Baustein,Konfiguration)))
Das Dokument <<KonzeptKonfiguration>> beschreibt, welche Arten von Konfiguration für eine Anwendung vorgesehen sind und wie diese umgesetzt werden sollen.
