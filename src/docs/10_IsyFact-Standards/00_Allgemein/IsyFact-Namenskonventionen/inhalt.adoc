...
Bei einem Maven-Artifact wird eine Group-ID und eine Artefakt-ID definiert. Die Group-ID gibt einen direkten Anhaltspunkt auf die Domäne.
...
Als Beispiel für diese Dokumentation wird `isy-documentation` als Artefakt-ID angegeben. Die Konkatinierung erfolgt über ein Miuns.
...
|*Schema Artifakt-ID* m|<präfix>-<funktion/komponente> +
