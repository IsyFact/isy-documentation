...
Beschäftigt man sich mit dem Entwurf und der Implementierung von Anwendungslandschaften, so stellt man fest, dass sich die einzelnen <<glossar-Fachanwendung,Fachanwendungen>> im Allgemeinen sehr ähnlich sind.
...
image::../../common/images/IFBereiche.png[align="center",pdfwidth=60%,width=60%]
...
*Blaupausen:* <<glossar-Blaupause,Blaupausen>> beschreiben die vorgegebene Architektur und Konzepte einer Anwendungslandschaft für den Betrieb von Fachanwendungen aus drei verschiedenen Sichten: der fachlichen Sicht, der Sicht der softwaretechnischen Umsetzung und der Sicht der technischen Infrastruktur.
...
Dieses Dokument ist übrigens V-Modell®-XT konform.
...
* *<<glossar-Fachanwendung,Fachanwendungen>>:* Eine Fachanwendung implementiert für sich genommen oder im Zusammenspiel mit anderen Fachanwendungen einen oder mehrere Geschäftsprozesse einer Anwendungsdomäne.
...
* *<<glossar-Querschnittssystem,Querschnittssysteme>>* sind Anwendungssysteme, die Basisdienste für mehrere Fachanwendungen bereitstellen, wie z. B. ein Behördenverzeichnis oder ein Outputmanagement.
...
Sie können aber auch jeweils nur Teile davon implementieren, sodass z.B. eine Fachanwendung eine GUI bereitstellt, eine weitere die Fachlogik implementiert und eine dritte schließlich die Persistierung der Daten realisiert.
...
* Einer zentralen Startseite, welche den eingeloggten Benutzern die Fachanwendungen der Anwendungslandschaft, für welche sie berechtigt sind, präsentiert.
...
Hierbei ist zu unterscheiden, ob ausschließlich IT-Systeme innerhalb der Systemlandschaft miteinander kommunizieren oder ob die Kommunikation auch IT-Systeme einschließt, die außerhalb liegen (siehe <<image-overkommIF>>).
...
Unabhängig von der Art der Kommunikation verwenden Services stets Schnittstellentechnologien, die HTTP als Übertragungsprotokoll einsetzen.

==== Kommunikation zwischen internen IT-Systemen

Innerhalb der Systemlandschaft kommunizieren IT-Systeme direkt, d.h. ohne ein System zur Vermittlung der ausgetauschten Nachrichten (wie bspw. ein Enterprise Service Bus).
Zur Realisierung dieser Kommunikation bietet die IsyFact Bausteine an, welche die entsprechenden Schnittstellentechnologien in die Referenzarchitektur integrieren.
...
Zur Abfederung von Lastspitzen kann z.B. ein Spooling über eine Queue eingesetzt werden.
...
Zur Implementierung der Geschäftslogik kann die Fachanwendung auch <<glossar-Service,Services>> anderer Fachanwendung aufrufen.
...
* Die Komponente _Batch_ der Fachanwendung wird im Beispiel nicht implementiert, da die Fachanwendung keine Schnittstelle für Batchläufe anbietet.
* Alle externen Aufrufe an die Fachanwendung werden durch _Service-Komponente_ verarbeitet: Keine andere Komponente darf externe Schnittstellen bereitstellen, insbesondere nicht in den _Anwendungskern_.
