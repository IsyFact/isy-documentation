...
* Einem Access-Manager, über den ein Single-Sign-On realisiert wird und in dem pro URL (und damit pro Geschäftsanwendung) konfiguriert ist, welche Berechtigungen für die Nutzung der Anwendung vorhanden sein müssen.
...
Es soll eine Geschäftsanwendung erstellt werden, die sowohl manuell über eine Weboberfläche als auch automatisiert über eine SOAP-Schnittstelle nutzbar sein soll.
...
* Die Schicht _Batch_ der Geschäftsanwendung wird im Beispiel nicht implementiert, da die Geschäftsanwendung keine Schnittstelle für Batchläufe anbietet.
* Alle externen Aufrufe an die Geschäftsanwendung werden durch die Schicht _Service_ verarbeitet: Keine andere Schicht darf externe Schnittstellen bereitstellen, insbesondere nicht die Schicht _Anwendungslogik_.
