...
Sie wurde entwickelt, um unterschiedliche <<glossar-Fachanwendung,Fachanwendungen>> effizient entwerfen, realisieren und betreiben zu können.
...
Sie umfassen allgemeingültige und wiederverwendbare Konzepte und Komponenten, die für die Entwicklung beliebiger Fachanwendungen relevant und verpflichtend sind.
...
Im vorliegenden Dokument verwenden wir daher den neutralen Begriff *<<glossar-Fachanwendung>>* für alle Arten von Anwendungssystemen, die eine bestimmte fachliche Funktionalität innerhalb einer Anwendungsdomäne realisieren.

*Querschnittssysteme* sind Anwendungssysteme, die <<glossar-Basisdienst,Basisdienste>> für mehrere Fachanwendungen bereitstellen.
...
Daher wird eine Referenzarchitektur aufgestellt, nach denen Anwendungssysteme (Fachanwendungen und Querschnittssysteme) software-technisch in <<glossar-IT-System,IT-Systeme>> umgesetzt werden.
