...
Das Projekt der Service-Schnittstelle benötigt die in Tabelle <<listing-bib-sst>> aufgelisteten Bibliotheken:
...
Eine vollständige Liste zeigt Tabelle <<listing-bib-sst-nutzung>>.
...
Zur Realisierung von Batches muss die in Tabelle <<listing-bib-batch>> aufgelistete Bibliothek eingebunden werden.
...
Um die Logging Funktionen in der eigenen Anwendung nutzen zu können müssen die in Tabelle <<listing-bib-logging>> aufgelisteten Bibliotheken eingebunden werden.
...
Zur Realisierung der Fehlerbehandlung und Implementierung von Exceptions müssen die in <<listing-bib-config>>
...
Zur Realisierung des AufrufKontext müssen die in <<listing-bib-aufrufkontext>> aufgelisteten Bibliotheken eingebunden werden.
