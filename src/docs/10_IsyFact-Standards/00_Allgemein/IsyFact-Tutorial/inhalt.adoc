...
+
NOTE: Eine Liste der angebotenen Metriken ist in <<KonzeptUeberwachung>> enthalten.
* Die von Anwendungen bereitzustellenden Informationen sind in <<KonzeptUeberwachung>> aufgeführt.
...
Diese besteht aus zwei Teilen, welche in <<KonzeptUeberwachung>> im Detail beschrieben sind:
...
Die Ping-Methode wird per HTTP Invoker als Service-Methode angeboten (siehe <<anbieten-von-service-schnittstellen>>).
