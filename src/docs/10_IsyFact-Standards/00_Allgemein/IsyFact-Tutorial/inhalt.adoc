...
NOTE: Dozer: Generischer Java-Bean zu Java-Bean Mapper (https://github.com/DozerMapper/dozer)
...
Dafür wird die Factory-Bean, die von isy-util mitgeliefert wird, genutzt (`de.bund.bva.pliscommon.util.mapper.DozerBeanMapperFactoryBean`) und in der Datei `src/main/resources/resources/spring/querschnitt.xml` als Spring-Bean konfiguriert.
Die zuvor angelegte Mapping-Dateien werden als Parameter übergeben.

:desc-listing-dozer-bean: Einbindung von Dozer mittels FactoryBean
[id="listing-dozer-bean",reftext="{listing-caption} {counter:listings}"]
.{desc-listing-dozer-bean}
[source,xml]
----
    <bean id="dozer" class="de.bund.bva.pliscommon.util.mapper.DozerBeanMapperFactoryBean">
