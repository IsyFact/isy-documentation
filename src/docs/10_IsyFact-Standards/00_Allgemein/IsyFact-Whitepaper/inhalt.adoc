...
|====
|Baustein|Funktion
|Fehler-behandlung |Der Baustein Fehlerbehandlung enthält das technische Konzept und eine Bibliothek zur einheitlichen Implementierung der Fehlerbehandlung für alle Anwendungen nach IsyFact-Architektur.
...
|Überwachung und Konfiguration |Der Baustein Überwachung und Konfiguration enthält ein technisches Konzept mit Designvorgaben und Bibliotheken für die Überwachung und die Handhabung von Konfigurationen für Anwendungen der IsyFact.
...
:desc-image-plattform: Bausteine der IsyFact
...
2+|IsyFact – Referenzarchitektur IT-System |PDF
...
3+|*10 Blaupausen*
.6+|*Technische Architektur* |Detailkonzept Komponente Web-GUI |PDF
|Detailkonzept Komponente Native-GUI |PDF
...
|*LDAP-Zugriffe* |Nutzungskonzept Spring LDAP |PDF
|*Logging* |Konzept Logging |PDF
|*Polling* |Nutzerdokumentation Polling |PDF
|*Portal* |Konzept Portal |PDF
.2+|*Sicherheits-komponente* |Nutzerdokumentation Sicherheit |PDF
|Rollen und Rechte Schema |XSD
.3+|*Sonderzeichen* |Konzept Umgang mit Sonderzeigen |PDF
|Mapping-Tabelle |XLS
|Transkriptionsregeln |XLS
|*Überwachung und + Konfiguration* |Konzept Überwachung und Konfiguration |PDF
|*Util* |Nutzungskonzept IsyFact Util |PDF
3+|*40 Methodik*
.3+|*Systemspezifikation* |Vorlage Systemspezifikation |DOT
|Vorlage Anforderungsliste |XSL
|Anleitung zu Datenflussdiagrammen |PDF
|*Systementwurf* |Vorlage Systementwurf |DOT
.2+|*Implementierung* |IsyFact Vorlage Systemhandbuch |DOT
