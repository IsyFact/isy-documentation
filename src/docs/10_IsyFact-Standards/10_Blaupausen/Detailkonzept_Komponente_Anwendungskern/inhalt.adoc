...
Die Fachkomponenten operieren auf Geschäftsobjekten (Entitäten).
Jedes Geschäftsobjekt ist genau einer Komponente zugeordnet, die die Datenhoheit über dieses Geschäftsobjekt besitzt und seinen Lebenszyklus verwaltet.
...
Das Interface realisiert damit eine Fassade.
...
1. Der zentrale Ausgangspunkt für die Spring-Konfiguration ist die Applikationsklasse der Anwendung (<<listing-applikationsklasse>>).
...
2. Für jede Schicht wird im Package `<org>.<domäne>.<anwendung>.<schicht>.config` eine mit `@Configuration` annotierte Konfigurationsklasse erstellt (<<listing-konfigurationsklasse>>).
...
Es existieren 3 verschiedene Verfahren der _Dependency Injection_:

- Constructor Injection
- Field Injection
- Method Injection

Da die _Constructor Injection_ diverse Vorteile gegenüber den beiden anderen Techniken liefert, ist die _Constructor Injection_ die bevorzugte Technik des _Dependency Injection_ in allen Projekten der Isy- und Register-Factory.

[[constructor-injection]]
==== Constructor Injection

Bei dem _Constructor Injection_ werden alle Abhängigkeiten einer Klasse über die Konstruktoren von außen injiziert.
...
Zusätzlich werden durch dieses Verfahren mögliche `NullPointerExceptions` vermieden.

_Constructor Injector_ hilft ebenfalls beim Identifizieren von zu vielen Abhängigkeiten zu einem Objekt.
...
Ist dies der Fall, müsste àn dieser Stelle über ein `Code-Refactoring` nachgedacht werden.

Ein weiterer Vorteil von _Constructor Injection_ ist, dass die injizierten Abhängigkeiten während der Laufzeit nicht veränderbar.
So wird die `Thread-Safe`-Eigenschaft der Abhängigkeiten sichergestellt.
