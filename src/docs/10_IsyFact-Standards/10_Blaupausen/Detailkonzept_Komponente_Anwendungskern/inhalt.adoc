...
[NOTE]
====
Die Abhängigkeiten des `JpaTransactionManager` kommen aus der Implementierung des Datenzugriffs.
Weitere Details sind im <<DetailkonzeptKomponenteDatenzugriff>> beschrieben.
====

//==== TX-Steuerung an Service-Schnittstellen
//
//an Implementiwird im Normalfall pro Service-Klasse vorgegeben.
