...
Deshalb soll in der `IsyHttpInvokerServiceExporter` Bean konfigurierte Service-Bean im eigentlichen Anwendungskontext
konfiguriert sein. `IsyHttpInvokerServiceExporter` ist eine Erweiterung von `HttpInvokerServiceExporter` und deaktiviert standardmäßig die Verwendung von Proxy-Klassen.
