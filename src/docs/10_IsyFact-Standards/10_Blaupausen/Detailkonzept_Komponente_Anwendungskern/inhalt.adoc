...
2. Für jede Schicht wird im Stammpackage `<org>.<domäne>.<anwendung>.<schicht>` parallel zu den Interfaces der Komponentenschnittstellen
eine mit `@Configuration` annotierte Konfigurationsklasse erstellt (<<listing-konfigurationsklasse>>).
...
* Bei HTTP Invoker Schnittstellen-Beans muss explizit eine Bean des eigentlichen Anwendungskontextes aufgerufen werden
(siehe <<NutzungsvorgabenHttpInvoker>>).
...
Sie besitzen ein statisches Attribut mit statischen `get`- und `set`-Methoden, die per Spring mit dem zur Verfügung
