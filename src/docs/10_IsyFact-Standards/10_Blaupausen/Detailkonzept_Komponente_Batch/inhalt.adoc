...
|`Anwendung.SpringDateien.<N>` |Vollqualifizierte Namen der Spring-Konfigurationsklassen der Fachanwendung
...
* Beans, die für die Batch-Verarbeitung nicht benötigt werden, sollten entfernt werden (HttpInvoker-Beans, GUI-Beans).
...
Damit werden mit `@Component` annotierte Klasse oder `@Bean`-Methoden in `@Configuration`-Klassen annotiert (<<listing-excludefrombatchcontext>>).
...
/// TODO Hierfür nehme man Datei resources/batch/rahmen/Batchrahmen.xml der Vorlageanwendung und passe die Konfiguration der Ausführungsbeans an.
