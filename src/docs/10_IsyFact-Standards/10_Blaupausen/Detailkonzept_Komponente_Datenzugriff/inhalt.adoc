...
Befindet sich die Anwendung aber in Produktion, dann muss der Parameter `spring.jpa.hibernate.ddl-auto` auskommentiert werden,
damit weder eine Generierung noch eine Validierung des Schemas stattfindet.
...
Für ein konkretes DAO ist eine eigenes Interface von der Basisschnittstelle `CrudRepository` abzuleiten.
...
Weiterhin ist das eigene Interface mit der Annotation `@Repository` zu versehen, damit alle vom Entity Manager erzeugten
...
1. Ableitung des Queries über den Namen der Methode. <<querymethodenname>> zeigt ein Beispiel hierfür.
+
...
verwendet wird und „Full Tablescans“ durchgeführt werden.
...
Teile dieser automatische Konfiguration können bei Bedarf überschrieben werden.
Soll z.B. für die Entwicklung eine andere Datenbank verwendet werden, kann die automatisch konfigurierte `DataSource`-Bean
durch eine andere überschrieben werden.
...
oracle.datasource.jdbc-timeout-connect=10000
...
Für n:1 Assoziationen wird genau so verfahren und das `fetch`-Attribut auf `FetchType.LAZY` gesetzt.
...
numerischen Versions-Felds in jeder Datenbank-Tabelle.
Dazu wird in den Entitäten eine numerische Property wird der Annotation `@Version` gekennzeichnet.
...
In der Regel läuft jeder Aufruf in einer eigenen Transaktion, weswegen kein Zustand und keine Daten zwischen zwei Aufrufen gehalten oder geteilt werden können.
Außer in Ausnahmefällen ist dies jedoch auch nicht notwendig.

Sollte es doch notwendig sein, sollte das Caching nicht in der Persistenzschicht und nicht mittels Hibernate geschehen.
...
Vertraulichkeit einzuhalten <<ITGrundschutz>>.
...
Das Verzeichnis enthält so immer das Installationsskript für die aktuelle Datenbankversion und Update-Skripte, um jede belieblige Vorversion seit Einführung der Schemaversionierung auf die aktuelle Datenbankversion anheben zu können.
...
[source]
----
oracle.datasource.schema-invalid-version-action=fail
oracle.datasource.schema-version=1.2.3
----
