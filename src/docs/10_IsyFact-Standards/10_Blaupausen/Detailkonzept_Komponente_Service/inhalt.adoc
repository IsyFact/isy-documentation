...
Dabei ist es zu beachten, dass die Annotation standardmäßig so konfiguriert ist, dass sie vor der Annotationen ohne Prioritätsangabe ausgeführt wird, so dass diese immer Zugriff auf die Korrelations-ID haben.
Diese Reihenfolge wird durch Einfügen des Orders für den Advisor `stelltLoggingKontextBereitAdvisor` in der Autokonfiguration `IsyServiceApiCoreAutoConfiguration` erreicht.
