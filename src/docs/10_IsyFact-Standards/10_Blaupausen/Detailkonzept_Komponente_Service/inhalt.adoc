...
NOTE: Für die Versionierung von Schnittstellen gelten gesonderte Vorgaben, die in <<IsyFactVersionskonzept>> definiert sind.

Zur Veröffentlichung von API-kompatiblen Änderungen wird im Maven `pom.xml` eine einstellige Versionsnummer gesetzt (vgl. <<beispiel-api-kompatible-aenderung>>).
...
[[beispiel-api-kompatible-aenderung]]
[source,xml]
----
<dependencies>
    ...
...
Bei inkompatiblen Änderungen der Schnittstelle wird die zweistellige Versionsnummer angepasst, diese wird sowohl in der Artifakt-ID als auch in den Paketnamen der Schnittstelle verwendet.
