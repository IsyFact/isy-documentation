...
1.  Konfiguration der Portalfarbe

In der Datei `Gruntfile.js` ist die Basisfarbe / Portalfarbe zu setzen:
...
In der Bibliothek `isy-style` ist ein "Maven install" durchzuführen. Dadurch wird ein "grunt build" ausgelöst und die Farbstyles befinden sich nach der Generierung
in `target/color.css`.

[start=3]
3.   Einbindung in Zielanwendung

Die Datei `color.css` aus dem Target-Verzeichnis von `isy-style` ist in die Zielanwendung unterhalb des WEB-INF Verzeichnisses zu kopieren.
