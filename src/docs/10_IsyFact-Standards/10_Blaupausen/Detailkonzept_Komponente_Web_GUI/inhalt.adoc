...
* `stepVisible`: Ob die Step Nummer gezeigt werden soll. (Standard: `true`, Typ: `Boolean` )
...
* `disabled`: Ob der Button deaktivert ist oder nicht. (Standard: `false`, Typ: `Boolean`)
* `showPrintView`: Ob die Komponente innerhalb einer Druckansicht angezeigt werden soll. (Standard: `#{not empty basisModel and basisModel.showPrintView}`, Typ: `Boolean`)
...
* `block`: AJAX: Ob ein Klick die GUI blockieren soll bis das Ergebnis eingetroffen ist. (Standard: `false`, Typ: `Boolean`)
* `defaultAction`: AJAX: Ob dieser Knopf die defaultAction des Formulars sein soll. (Standard: `false`, Typ: `Boolean`)
...
:desc-image-buttonRow: Beispiele für <isy:buttonRow>
...
* `alignRight`: Ob die Button Row ein Alignment nach Rechts besitzen soll. (Standard: `false`, Typ: `Boolean`)
...
* `alignRight`: Ob die Button Row eine Alignment nach Rechts besitzen soll oder nicht. (Standard: `false`, Typ: `Boolean`)
* `alignLeft`: Ob die Button Row eine Alignment nach Links besitzen soll oder nicht. (Standard: `false`, Typ: `Boolean`)
...
* `showIcon`: Ob ein Icon angezeigt werden soll oder nicht. (Standard: `true`, Typ: `Boolean`)
* `reverseIconPosition`: Ob das Icon rechts angezeigt werden soll. Standard: false. (Standard: `false`, Typ: `java.lang.Boolean`)
* `disabled`: Ob der Button deaktiviert sein soll. Standard: false. (Standard: `false`, Typ: `java.lang.Boolean`)
* `showPrintView`: Ob der Button innerhalb einer Druckansicht angezeigt werden soll. (Standard: `#{not empty basisModel and basisModel.showPrintView}`, Typ: `Boolean`)
...
* `reverseIconPosition`: Ob das Icon rechts angezeigt werden soll. Standard: false. (Standard: `false`, Typ: `java.lang.Boolean`)
* `disabled`: Ob der Button deaktiviert sein soll. Standard: false. (Standard: `false`, Typ: `java.lang.Boolean`)
* `active`: Ob der Button ausgewählt sein soll. Standard: false. (Standard: `false`, Typ: `java.lang.Boolean`)
* `execute`: AJAX: Welcher Template-Seitenbereich ausgeführt werden soll. (Standard: `@form`)
* `render`: AJAX: Welcher Template-Seitenbereich gerendert werden soll. (Standard: `@form`)
* `globalConfig`: Eine spezifische globale Konfiguration, falls nötig. (Standard: `#{globalConfigurationModel}`)
* `showPrintView`: Ob die Komponente innerhalb einer Druckansicht angezeigt werden soll. (Standard: `#{not empty basisModel and basisModel.showPrintView}`, Typ: `Boolean`)
...
* `showIcon`: Ob das Icon angezeigt werden soll oder nicht. Standard: true. (Standard: `true`, Typ: `java.lang.Boolean`)
...
* `openInNewTab`: Ob der Link in einem neuen Tab geöffnet werden soll. Funktioniert nur mit aktiviertem JavaScript. (Standard: `false`, Typ: `Boolean`)
...
* `skipAction`: Ob die übergebene `action` ausgelassen werden soll. Default ist false, die `action` wird standardmäßig also ausgeführt. (Standard: `false`, Typ: `java.lang.Boolean`)
...
* `tabController`: Ein spezifischer Tab-Controller, falls notwendig. (Standard: `#{tabController}`)
* `preload`: Ob das Tab im JS-Fall vorgeladen werden soll. Default ist false, so dass nicht vorgeladen wird. (Standard: `false`, Typ: `java.lang.Boolean`)
...
* `selectable`: Ob die Datatable selektierbar ist oder nicht. Diese Information wird benötigt, um das JavaScript Plug-In zu konfigurieren. (Standard: `false`, Typ: `Boolean`)
...
* `execute`: AJAX: Welcher Template-Seitenbereich ausgeführt werden soll. (Standard: `@form`)
* `render`: AJAX: Welcher Template-Seitenbereich gerendert werden soll. (Standard: `@form`)
* `highlightDoubleclick`: Ob Zeilen bei einem Doppelklick hervorgehoben werden sollen. (Standard: `false`, Typ: `Boolean`)
...
* `showMore`: Ob ein Button zum Anzeigen von mehr Spalten angezeigt werden soll. (Standard: `false`, Typ: `Boolean`)
* `styleClass`: Die zu ergänzenden Style-Klassen.
...
* `hidden`: Ob die Spalte angezeigt werden soll. (Standard: `false`, Typ: `Boolean`)
* `id`: Identifier.
...
* `align`: ibt die horizontale Ausrichtung des Inhalts der Tabellenzelle an; mögliche Werte sind left, right und center; (Standard: `left`)
...
* `dropdown`: Ob eine Auswahlliste zur Filterung angezeigt werden soll, falls ja müssen die Auswähleinträge mittels selectItem(s) angegeben werden. (Standard: `false`, Typ: `Boolean`)
...
* `disabled`: Ob die Checkbox deaktiviert ist oder nicht. (Standard: `false`, Typ: `Boolean`)
* `required`: Ob die Checkbox zwingend markiert werden muss oder nicht. (Standard: `false`, Typ: `Boolean`)
...
* `labelRight`: Der Text für das Label rechts neben
...
* `required`: Ob die Eingabe ein Pflichteingabe ist. (Standard: `false`, Typ: `Boolean`)
* `optional`: Ob die Eingabe eine Optionale-Pflichteingabe ist. (blaues Sternchen) (Standard: `false`, Typ: `Boolean`)
...
Außerdem gibt es folgende optionale Parameter:
* `disabled`: Ob die Darstellung nur lesend erfolgen soll (Standard: `false`, Typ: `Boolean`)
...
Außerdem gibt es folgende optionale Parameter:
* `customId`: Erlaubt, eine spezielle ID zu setzen, um die Komponente z.B. für JavaScript oder Tests zugreifbar zu machen. Diese wird im Attribut data-isy-custom-id eingetragen.
* `inline`: Ob die Radio-Buttons inline angezeigt werden sollen. (Standard: `false`, Typ: `Boolean`)
* `disabled`: Ob die Darstellung nur lesend erfolgen soll. (Standard: `false`, Typ: `Boolean`)
* `label`: Der Text für das Label.
* `labelStyleClass`: Die CSS-Klasse für das Label. (Standard: `col-lg-6`)
* `inputStyleClass`: Die CSS-Klasse für den Eingabebereich. (Standard: `col-lg-6`)
...
* `disabled`: Ob die Liste deaktiviert ist. (Standard: `false`, Typ: `Boolean`)
...
* `collapse`: Ob das Panel einklappbar sein soll. Falls ja, so muss auch ein panelModel angegeben werden. (Standard: `false`, Typ: `Boolean`)
...
* `headerPanel`: Ob die Gruppe als Container für den Header des Inhaltsbereichs dient; falls ja werden entsprechende CSS-Klassen für das Layouting eingebunden. (Standard: `false`, Typ: `java.lang.Boolean`)
...
* `inputStyleClass`: CSS-Klasse für das Eingabefeld (nicht implementiert)
...
* `required`: Ob die Eingabe ein Pflichteingabe ist. (Standard: `false`, Typ: `Boolean`)
* `readonly`: Ob die Darstellung nur lesend erfolgen soll (Standard: `false`, Typ: `Boolean`)
* `label`: Der Text für das Label
* `labelStyleClass`: Die CSS-Klasse für das Label. (Standard: `col-lg-6`)
* `inputStyleClass`: Die CSS-Klasse für den Eingabebereich. (Standard: `col-lg-6`)
* `placeholder`: Der Platzhalter, welcher im Eingabefeld angezeigt wird.
* `disabled`: Ob das Eingabefeld deaktiviert ist. (Standard: `false`, Typ: `Boolean`)
...
* `fourEyesLastValue`: Letzter Wert im 4-Augen-Eingabefeld
* `alignright`: Ob der Text innerhalb des Eingabefeldes rechts ausgerichtet sein soll. (Standard: `false`, Typ: `Boolean`)
...
* `inputmaskInsertMode`: Die Eingabemaske: ob text soll überschrieben werden beim eintippen.(Standard: `true`, Typ: `Boolean`)
* `inputmaskClearOnBlur`: Die Eingabemaske: Ob beim Verlassen des Feldes die Maskierungsplatzhalter ausgeblendet werden sollen (Standard: `true`, Typ: `Boolean`)
* `showPicker`: Ob Picker angezeigt werden soll. (Standard: `true`, Typ: `java.lang.Boolean`)
* `validationModel`: Ein spezifisches Validation-Model, falls benötigt. (Standard: `#{validationModel}`)
* `globalConfig`: Eine spezifsche globale Konfiguration, falls benötigt. (Standard: `#{globalConfigurationModel}`)
* `fourEyesLastValue`: Letzter Wert im 4-Augen-Eingabefeld
...
* `tausenderPunkt`: Ist ein Tausender-Punkt gewünscht. (Standard: `false`, Typ: `Boolean`)
* `fourEyesLastValue`: Letzter Wert im 4-Augen-Eingabefeld
* `alignright`: Ob der Text innerhalb des Eingabefeldes rechts ausgerichtet sein soll. Default ist false.(Standard: `false`, Typ: `Boolean`)
...
:desc-image-demopanelformSelectOneDropdown: WebGui-Demoanwendung: isy_formSelectOneDropdownWithFourEyes Varianten
[id="image-demopanelformSelectOneDropdown",reftext="{figure-caption} {counter:figures}"]
.{desc-image-demopanelformSelectOneDropdown}
image::demopanel-formSelectOneDropdown.png[align="center"]
...
* `invalid`: Ob die Auswahl invalide ist oder nicht. (Standard: `false`, Typ: `Boolean`)
* `customId`: Erlaubt, eine spezielle ID zu setzen, um die Komponente z.B. für JavaScript oder Tests zugreifbar zu machen. Diese wird im Attribut data-isy-custom-id eingetragen.
* `readonly`: Ob die Darstellung nur lesend erfolgen soll (Standard: `false`, Typ: `Boolean`)

NOTE: Besonderheit: Das formSelectOneDropDownWithFourEyes-Widget wird im Modus `readonly="true"` als Label gerendert.
...
* `customId`: Erlaubt, eine spezielle ID zu setzen, um die Komponente z.B. für JavaScript oder Tests zugreifbar zu machen. Diese wird im Attribut data-isy-custom-id eingetragen.
* `required`: Ob die Eingabe ein Pflichteingabe ist. (Standard: `false`, Typ: `Boolean`)
* `disabled`: Ob die Bearbeitung aktiviert ist oder nicht(Standard: `false`, Typ: `Boolean`)
* `readonly`: Ob auf das Feld nur lesend zugegriffen wird (Standard: `false`, Typ: `Boolean`)
* `label`: Der Text für das Label
* `labelStyleClass`: Die CSS-Klasse für das Label. (Standard: `col-lg-6`)
* `inputStyleClass`: Die CSS-Klasse für den Eingabebereich. (Standard: `col-lg-6`)
...
* `invalid`: Ob die Auswahl invalide ist oder nicht. (Standard: `false`, Typ: `Boolean`)
* `disabled`: Ob die Auswahl deaktiviert ist oder nicht. (Standard: `false`, Typ: `Boolean`)
...
[[FocusOnload]]
=== FocusOnload
<isy:focusOnload> -  Dient dazu, das initial zu fokussierende Element einer Seite anzugeben oder die initiale Fokussierung zu deaktivieren.
...
[[specialCharPicker]]
=== Sonderzeichen Widget
<isy:specialCharPicker> -  Diese Komponente dient dazu, die Eingabe der nationalen Buchstaben und Sonderzeichen über ein externes Widget-Plugin zu ermöglichen/erleichtern.

Die Komponente besitzt weder notwendige noch optionale Parameter.
