...
IMPORTANT: Um die Kapselung der GUI-Komponenten zu bewahren, ist es wichtig, dass GUI-Komponenten ihre Parameter immer über ein Input-Tag erhalten und nicht frei auf fremde Models und Controller zugreifen.
...
* `styleClass`: zu ergänzende Style-Klassen
* `alignRight`: ob die Button Row rechtsbündig ist (Standard: `false`)
