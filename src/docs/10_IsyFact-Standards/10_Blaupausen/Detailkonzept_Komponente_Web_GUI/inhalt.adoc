...
*Java-Bibliothek `isy-style`*

Für die Erstellung von Weboberflächen existieren Vorgaben in Form eines Styleguides <<Styleguide>>. Darin
werden Anforderungen an die Gestaltung und Nutzbarkeit von Oberflächen festgelegt. In der `isy-style` werden die CSS-, JavaScript- und Bilddateien des Styleguides abgelegt und für die Nutzung komprimiert und kompiliert.

Das CSS für eine Styleguide konforme Anwendungsentwicklung wird mit der Bibliothek `isy-style` automatisch geladen und eingebunden.
...
*Java-Bibliothek `isy-web`*

Die Java-Bibliothek `isy-web` enthält notwendige Abhängigkeiten (Maven), Spring-Konfigurationen, JSF-Komponenten (Composite Components) sowie zugehörige JSF-Templates (.xhtml) und setzt die Styleguide Vorgaben durch Verwendung der Java Bibliothek `isy-style` um.
Die Bibliothek `isy-web` kann über das Build-Tool Maven eingebunden werden. Die Maven Abhängigkeit für die `isy-web` lautet:
...
Das Model wird als Variable definiert und damit im Flow erzeugt (`auskunftModel`). In der on-start Definition wird der Controller aufgerufen, um das Model zu initialisieren.
...
<flow
  ...
  abstract="true">
	...
	<!-- Erzeugung des auskunftModels. -->
	<var name="auskunftModel"
		class="de.packagepfad.AuskunftModel" />

	<var name="abcModel"
		class="de.packagepfad.MeldungWizardModel" />

	<!-- Initialisieren des auskunftModels. -->
	<on-start>
	  <evaluate
       expression="auskunftController.setzeStartseite(auskunftModel)"/>
	  <evaluate
	   expression="auskunftController.initModel(auskunftModel)"/>
	...
	<view-state id="auskunftViewState" model="auskunftModel">
	  <on-render>
        <evaluate
		 expression="auskunftController.initViewState(auskunftModel)">
		</evaluate>
	  </on-render>
	...
	<transition on="sucheAbc" to="_abcsuche">
	  <evaluate
	   expression="auskunftController.fuehreAuskunftDurch(auskunftModel)" />
	</transition>
	<transition on="sucheAbc" to="_abcsuche">
      <evaluate expression="..." />
    </transition>
...
(z.B. `auskunftViewState.xhtml`), der automatisch beim Rendern aufgerufen wird.
...
:desc-listing-jspViewState: Beispiel für eine JSP ViewState Seite
...
<ui:composition xmlns="http://www.w3.org/1999/xhtml"
	...
	<isy:buttonIcon id="druck_icon"
	   ...
	   size="large"   action="drucken"
	   reverseIconPosition="true" ajax="false">
	</isy:buttonIcon>
...
TIP: Die IsyFact enthält die Demo-Anwendung `isy-webgui`.
     ...
     Insbesondere demonstriert sie Aussehen und Verhalten der nachfolgend beschriebenen JSF-Seitenelemente/JSF-Bedienelemente.
...
Es muss weiterhin ein Controller erstellt werden, der vom `AbstractGuiController` erbt und ein Model, das vom `AbstractMaskenModel` erbt. Die XHTML-Seiten der ViewStates müssen alle vom JSP-Masken-Template `/WEB-INF/gui/common/layout/applikation.xhtml` erben (s.a. <<listing-jspViewState>>).
...
Die XHTML-Seiten der ViewStates müssen alle vom JSP-Masken-Template `/WEB-INF/gui/common/layout/applikationDetailseite.xhtml` erben.
...
kann weiterhin definiert werden, welche Inhalte zusätzlich direkt rechts neben der Headline (Titelzeile) angezeigt werden bzw.
ganz am rechten Rand der Headline angezeigt werden. Im Folgenden ein Beispiel: 
...
* `ajax`: AJAX: Ob die Aktion per AJAX ausgeführt werden soll oder nicht
...
* `defaultAction`: Ob der Button der Default-Button sein soll.
* `globalConfig`: Eine spezifische globale Konfiguration, falls notwendig
...
* `fourEyesLastValueesMode`: 4 Augen Prinzip Mode (locked, unlock)
...
* `globalConfig`: Eine spezifische globale Konfiguration, falls notwendig.
* `skipAction`: Ob die übergebene action ausgelassen werden soll. Default ist `false`, die action wird standardmäßig also ausgeführt.
* `action`: Die action kann überschrieben werden, um z.B. den aktuellen Reiter validieren zu können.
...
Eine Liste mit Einfachauswahl wird mit dem Tag `<isy:formSelectOneList>` eingebunden.
...
Eine Liste mit Mehrfachauswahl wird mit dem Tag `<isy:formSelectManyList>` eingebunden.
...
Beim Initialisieren des Models muss der Operationsmodus (SERVER oder CLIENT) angegegben werden.
...
Für Formulare existiert das Tag `<isy:formCheckbox>` und bietet vor allem eine Integration auf einem Formular.
...
* `disabled`: Ob die Darstellung nur lesend erfolgen soll
...
Jede Anwendung muss daher die Ressource `/WEB-INF/gui/common/seitenelemente/navigation.xhtml` mit einer spezifischen Anpassung überschreiben.
...
Für Quicklinks kann es nun auch mehrere Gruppen geben (z.B. aktuelle Suchanfrage, letzte Suchanfragen)

Gruppen von Quicklinks können jetzt abhängig vom aktuellen Flow angezeigt werden.
...
Ein Charpicker für Sonderzeichen kann durch die Angabe von `charpicker=true` in einem Formulareingabefeld (`<isy:formInput>`) aktiviert werden.
...
* `inputComplementer`: Die Nummer der Headerspalte, dessen Inhalt per Javascript im Eingabefeld ergänzt werden soll, sobald das Feld verlassen wird (z.B. Schlüsselwert zu Schlüssel anzeigen). Standardmäßig 0 = deaktiviert.
...
Browse and Collect kann über das Tag `<isy:formBrowseAndCollect>` und `<isy:browseAndCollect>` eingebunden werden.
...
* `panelModel`: Das Panel Model. Nur Notwendig falls das Panel einklappbar sein soll.
...
In den Formularen kann durch das Angeben von `<isy:formDefaultButton> eine Standardbutton zur Auslösung definiert werden.
Beim Drücken auf "Enter" wird dann diese Aktion ausgeführt.
Die Funktionalität ist nur bei aktiviertem JavaScript verfügbar. 

Um anwendungsspezifische Elemente in ein Formular zu integrieren, kann das Tag `<isy:formCustom>` verwendet werden.
...
Für das einbinden eines Tooltips kann das Tag `<isy:tooltip>` verwendet werden.
Es besitzt folgende Parameter:
...
Die Darstellung von Fehlern wird von der Bibliothek `isy-web` über das JSP-Seitentemplate `basis.xhtml` vorgenommen.
Die Fehlermeldungen werden hierbei im Kopfbereich des Applikations-Inhaltsfensters xxxx angezeigt.
...
So können checked und unchecked Exceptions abgefangen werden. Der `MessageController` bietet hierzu Methoden
(writeException, writeAndLogException) zum Loggen und Erzeugen von Fehlern/Warnungen im Nachrichtenbereich an.
...
----
* In der `faces-config.xml` (Java Server Faces Konfigurationsdatei) muss der JSF Exception Handler konfiguriert werden:
...
GUI-Konzept einzurichten. Konkrete Beispiele finden sich in der <<Vorlageanwendung>> und den nachfolgenden Beispielen.

[[web.xml]]
== Web Deployment Deskriptor

Die Datei `web.xml` ist der Web Deployment Deskriptor und enthält die notwendigen Konfigurationen, um die Teilsysteme (JSF, Spring Web Flow, Security) zu aktivieren.
...
Aus Sicht der Styleguide-Umsetzung ist die Konfiguration des ApplicationIntialisierungFilter besonders wichtig.
...
:desc-listing-jspWebXmlKonfigAppInitFilter: web.xml Konfiguration für den ApplicationInitialisierungsFilter
[id="listing-jspWebXmlKonfigAppInitFilter",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-jspWebXmlKonfigAppInitFilter}
[source, xml]
----
...
[[zentraleSpringKonfigurationen]]
== Zentrale Spring Konfigurationen

In der Datei `application.xml` sind alle zentralen Spring Konfigurationen aufgeführt.
