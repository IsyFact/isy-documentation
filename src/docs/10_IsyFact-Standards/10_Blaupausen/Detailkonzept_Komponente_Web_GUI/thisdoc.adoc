...
Im Dokument <<IsyFactReferenzarchitekturITSystem>> ist der Aufbau von <<glossar-Fachanwendung>>en in fünf Komponenten beschrieben.
...
Es werden die Randbedingungen und Besonderheiten für deren Einsatz innerhalb einer <<glossar-Anwendungslandschaft>>, die nach den <<glossar-IFS>> gebaut wird, erläutert.
