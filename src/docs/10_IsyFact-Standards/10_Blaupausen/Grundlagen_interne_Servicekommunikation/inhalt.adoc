...
Gemäß der Referenzarchitektur (siehe <<IsyFactReferenzarchitektur>>) kommunizieren <<glossar-Anwendungssystem>>e auf Basis von Services.
...
Gemäß der <<IsyFactReferenzarchitektur>> ist der Zugriff von Service-Gateways auf <<glossar-Querschnittssystem>>e erlaubt.
...
*Hohe Verfügbarkeit:* Die <<glossar-Fachanwendung>>en der <<glossar-Plattform>> müssen eine hohe Verfügbarkeit aufweisen.
