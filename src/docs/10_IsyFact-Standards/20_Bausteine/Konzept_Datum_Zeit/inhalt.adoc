...
Die Persistierung muss sich nahtlos in die anderen Bausteinen der IsyFact integrieren, vollständig kompatibel mit dem <<DetailkonzeptKomponenteDatenzugriff>> sein und gemäß den Vorgaben des Produktkatalogs umgesetzt werden.
...
Das Stammpackage für alle Klassen der Umsetzung lautet: `de.bund.bva.isyfact.datetime`.
...
Bisherige Implementierungen ungewisser Datumswerte setzen immer den Sonderfall um, bei dem entweder der Tag, der Tag und der Monat oder alle Teile eines Datumswerts unbekannt sind.
