...
<<listing-MappingPlisNachPlisTo>> zeigt ein Beispiel für das Mappen einer technischen `PlisException` in eine technische Transport-Exception:

:desc-listing-MappingPlisNachPlisTo: Mappen einer PlisException in eine technische Transport-Exception
[id="listing-MappingPlisNachPlisTo",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-MappingPlisNachPlisTo}
[source,java]
----
...
Wie in <<listing-FehlerSchluessel>> gezeigt, müssen die Konstanten einen sprechenden Namen tragen und immer mit `MSG_` beginnen.
