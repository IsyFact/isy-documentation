...
Die Verknüpfung von Anwendungsfehlern über mehrere Anwendungen bzw. deren Log-Dateien hinweg geschieht über zwei IDs, die in dem Dokument <<NutzungsvorgabenFehlerbehandlung>> beschrieben sind.
...
Diese werden im Dokument <<NutzungsvorgabenFehlerbehandlung>> beschrieben.
