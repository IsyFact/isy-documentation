...
Um die Kompatibilität zu weiteren IAM-Systemen (siehe <<KonzeptSicherheit>>) zu gewährleisten, werden systemspezifische Daten des IAM-Systems von der HTTP Invoker Schnittstelle verarbeitet und weitergeleitet.

Bei der Verarbeitung werden die systemspezifischen Daten im AufrufkontextVerwalter abgelegt sind so jeder Methode des Remote-Beans zugänglich (siehe <<aufbau-des-remote-beans>>).

Die Weiterleitung dieser Daten erfolgt über den HTTP-Header des HTTP-Request.

[[aufbau-der-service-logik]]
== Aufbau der Service-Logik

Die Service-Logik besteht, wie im <<DetailkonzeptKomponenteService>> beschrieben, aus einem Remote-Bean, einer Exception-Fassade und einer Service-Fassade pro Service.
