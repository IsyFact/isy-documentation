...
Um die Kompatibilität zu OAuth 2 (siehe <<KonzeptSicherheit>>) zu gewährleisten, wird das Bearer Token von allen HTTP Invoker Schnittstellen verarbeitet und weitergeleitet.
...
Die Weiterleitung des Bearer Tokens erfolgt über einen HTTP-Header, sodass es für existierende Schnittstellen transparent erfolgt.
