...
In der Regel erfolgt die Authentifizierung von <<glossar-Anwender>>n über eine zentrale Komponente der Systemlandschaft, in welche das IT-System eingebettet ist.
...
Für <<glossar-Service>>s des Querschnitts, die nahezu alle Aufrufe benötigen und die keine sicherheitskritischen Operationen anbieten, kann eine zentrale Rolle (z.B. `Querschnitt_Nutzer`) angelegt werden.
...
<<glossar-IT-System>>e werden intern in Form von Batches oder Timer-Tasks aktiv.
