...
Die Mechanismen zur Absicherung IsyFact-konformer IT-Systeme hat zum Ziel, die Autorisierung von Zugriffen systematisch, einheitlich und einfach umzusetzen.
...
Die Informationen aus dem HTTP-Header werden in die als `AufrufKontext` in das IT-System übernommen.
...
* Einem innerhalb der Logik- und Verarbeitungszone eines IT-Systems übergebenen `AufrufKontext` kann vertraut und ohne erneute Rückfrage an das IAM-System bzw. die lokale Authentifizierung verwendet werden.
...
So wird kann über Interceptoren der `AufrufKontext` bei Serviceaufrufen transparent über Spring AOP gesetzt werden.
...
* Für den Kontext der Anfrage stellt die Komponente einen Berechtigungsmanager zur Verfügung, der die Rollen des anfragenden Anwenders kennt.
...
Die Außensicht der Komponente ist folgendermaßen aufgebaut:
