...
Die konkrete Sicherheitsarchitektur einer <<glossar-Systemlandschaft>> muss nach folgenden Prinzipien gestaltet sein:
...
In der Regel erfolgt die Authentifizierung von <<glossar-Anwender,Anwendern>> über eine zentrale Komponente der Systemlandschaft, in welche das IT-System eingebettet ist.
...
Im Folgenden werden die Aufgaben und grobe Funktionsweise der Komponenten für die Autorisierung von Anfragen in einer <<glossar-Fachanwendung>> erläutert.
...
Für die Technologie Spring HttpInvoker stellt die IsyFact passende Transportobjekte in der Bibliothek `isy-serviceapi-sst` bereit.


[[rollen-und-rechte]]
== Rollen & Rechte

Die Vergabe von Rollen ist _das_ Mittel der Benutzeradministration, um Anwender der <<glossar-Anwendungslandschaft>> mit Berechtigungen auszustatten.
Die Vergabe von Rollen an einen Anwender (menschliche und technische) erfolgt im Querschnitt:
entweder in der Querschnittsanwendung Benutzerverzeichnis oder in der <<glossar-Querschnittskomponente>> Benutzerverwaltung.
...
:desc-image-rollen-beziehungen: Beziehungen zwischen fachlichen und technischen Rollen
[id="image-rollen-beziehungen",reftext="{figure-caption} {counter:figures}"]
.{desc-image-rollen-beziehungen}
...
Sie werden für die Schnittstellen von Fachanwendungen verwendet, welche nur von anderen Fachanwendungen aufgerufen werden.
Die einzelnen Schnittstellen werden durch Rechte abgesichert (siehe <<NutzungsvorgabenSicherheit>>).

Technische Rollen berechtigen zur Ausführung der entsprechenden Services der Fachanwendung selbst, sowie aller dadurch mittelbar ausgelösten Aktionen in nachgelagerten Anwendungen.
...
Anwender gelangen in der Regel entweder über das <<glossar-Portal>> oder den <<glossar-Service-Gateway>> in eine Anwendungslandschaft.
...
Für <<glossar-Service,Services>> des Querschnitts, die nahezu alle Aufrufe benötigen und die keine sicherheitskritischen Operationen anbieten, kann eine zentrale Rolle (z.B. `Querschnitt_Nutzer`) angelegt werden.
...
Fachliche Rollen werden für Schnittstellen von Fachanwendungen vergeben, welche Zugänge zur Anwendungslandschaft geben.
...
Die einzelnen angebotenen Services werden über Rechte abgesichert (siehe <<NutzungsvorgabenSicherheit>>).
...
Der fachliche Systemname beschreibt die Fachanwendung, bzw. die Anwendungsdomäne, in welcher die entsprechende Funktionalität bereitgestellt wird.
Er entspricht prinzipiell dem Systemnamen der Systemspezifikation, abzüglich technischer Kürzel.
Die Rolle zur Verwendung der Schnittstelle Auskunft der Fachanwendung Terminfindung lautet nach diesem Schema: `Terminfindung_Auskunft`.
...
<<glossar-IT-System,IT-Systeme>> werden intern in Form von Batches oder Timer-Tasks aktiv.
...
Die Rolle zur Verwendung der Schnittstelle "Eintragen der Teilnahme" der Fachanwendung Terminfindung lautet nach diesem Schema: `Terminfindung-FA_TeilnahmeEintragen`.
...
:desc-image-rollen-erstellung: Absicherung durch Rechte und Aggregation in Rollen
[id="image-rollen-erstellung",reftext="{figure-caption} {counter:figures}"]
.{desc-image-rollen-erstellung}
...
. Konfiguration des IT-Systems mit den erstellten technischen Rollen und Rechten (siehe <<NutzungsvorgabenSicherheit>>).
...
Es gestaltet sich bislang schwer, dass vor der eigentlichen Inbetriebnahme nur eine kleine Menge von Anwendern auf eine neue Fachanwendung zugreifen kann.
So werden oft, auch bei der Ablösung einer Fachanwendung durch eine neue Umsetzung, komplett neue Rollen für die neue Fachanwendung vergeben, um die Absicherung beider Fachanwendungen zu gewährleisten.
...
Fachanwendungen, die bestehende Fachanwendungen ablösen oder vor der offiziellen Inbetriebnahme einer kleinen Menge von Anwendern zur Verfügung stehen, müssen in ihrer betrieblichen Konfiguration einen Schalter besitzen, der einen Testmodus aktiviert.
...
Somit ist sichergestellt, dass beim Ablösen von alten Fachanwendungen auch die neue Fachanwendung mit denselben Rollen abgesichert und (falls nötig) parallel betrieben werden kann.
Genauso funktioniert auch das Freischalten einer neuen Fachanwendung für einen zunächst kleinen Kreis von Anwendern.
