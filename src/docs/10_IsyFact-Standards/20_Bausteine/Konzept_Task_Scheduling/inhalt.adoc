...
Dazu ist ein Benutzername und ein Passwort nötig, dass die Bibliothek als betriebliche Konfiguration, wie im Baustein Konfiguration (s. <<KonzeptUeberwachungKonfiguration>>) vorgegeben, einlesen muss.
...
Die Definition der Standardwerte ergänzt die Regelungen zu Default-Werten in der betrieblichen Konfiguration (s. <<KonzeptUeberwachungKonfiguration>>).
