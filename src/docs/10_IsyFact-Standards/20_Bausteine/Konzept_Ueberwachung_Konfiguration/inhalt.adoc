...
Dies geschieht in der <<Vorlageanwendung>> in einem `WatchdogTask` der von `isy-ueberwachung` bereits angeboten wird.
Dieser Task wird ebenfalls als Bean deklariert und beinhaltet den TerminfindungWatchdog (Bean-ID=administration), wie folgt:
...
[source,xml]
----
<bean id="watchdogTask" class="de.bund.bva.pliscommon.ueberwachung.task.AdministrationWatchdogTask">
    <constructor-arg ref="watchdogTask-monitor"/>
    <property name="administrationWatchdog" ref="administration"/>
</bean>
...
Die Properties müssen die Bezeichnung `watchdogTask` beinhalten, da dies die Bean-ID von dem WatchdogTask aus
obiger Konfiguration ist.
