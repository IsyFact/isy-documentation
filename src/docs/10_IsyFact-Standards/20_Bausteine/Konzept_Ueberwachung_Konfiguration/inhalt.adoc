...
NOTE: Die Komponenten einer IsyFact-Anwendung werden in <<IsyFact-Referenzarchitektur>> beschrieben.
...
Tabelle <<table-Stdueber>> zeigt eine Liste der zu überwachenden bzw.
...
Tabelle <<table-StdueberServ>> zeigt eine Liste der zu überwachenden bzw.
anzubietenden Informationen für das Management-Interface von Service-Anwendungen.
Die in Tabelle 2 aufgelisteten Informationen müssen für jeden Service einzeln angeboten werden.
...
gemeinsamen Advisor mit einem Pointcut zu verwenden, der die verschiedenen Ziele mit `or` verknüpft, wie im folgenden Beispiel dargestellt:
...
Mit nachfolgender Spring-Konfiguration werden Konfigurationen und Statistiken zu dem in `classpath:ehcache.xml`
...
Um dies zu vermeiden, sollte zur Prüfung der genutzten Ressourcen das Future-Pattern verwendet werden:
+
[NOTE]
====
...
Dazu ist die folgende Konfiguration notwendig:
...
Dazu ist der folgende Abschnitt in die Spring-Konfiguration aufzunehmen:
...
Die `demoBean` wird wie eine gewöhnliche Spring-Bean definiert, muss jedoch, zur Einhaltung der
...
Dazu wird ein MBeanInfoAssembler wie folgt konfiguriert:
...
die Teil des Management-Interfaces werden sollen, mit der Annotation `@ManagedAttribute`
bzw. `@ManagedOperation` versehen:
...
Der Zugriff auf eine MBean erfolgt dabei nach folgendem Muster:
...
Konfigurationen lassen sich wie in Tabelle <<table-typKonfZiel>> dargestellt typisieren:
...
Die Konfiguration wird der Anwendung als querschnittliche Spring-Bean (im Folgenden Konfigurations-Bean genannt) bereitgestellt:
...
über das Interface `de.bund.bva.pliscommon.konfiguration.common` einen typsicheren Zugriff auf die
...
Das Interface bietet für verschiedene Datentypen (String, Integer, Long, Double und Boolean) jeweils
typsichere Zugriffsmethoden an.
