...
Die folgenden Metriken durch Spring Boot automatisch registriert und müssn bereitgestellt werden:
...
Für Caches, die beim Start der Anwendung zur initialisiert sind, werden automatisch im Endpoint `metrics` Statistiken
...
Zusätzlich zu den durch eingauten Metriken kann eine Anwendung selber Metriken aufzeichnen und über Micrometer registrieren.
...
[id="listing-endpointnfig",reftext="{listing-caption} {counter:listings }"]
