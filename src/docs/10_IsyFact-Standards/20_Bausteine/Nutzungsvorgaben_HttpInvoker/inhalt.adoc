...
[NOTE]
====
Wichtige ergänzende Hinweise sowie eine sehr umfangreiche Anleitung zur Erstellung von Service-Schnittstellen mit HTTP Invoker bietet das <<IsyFactTutorial>>.
====
...
Für diese Transport-Exceptions gilt über die Vorgaben des <<KonzeptFehlerbehandlung>> hinaus noch:
...
:desc-image-010: Vererbungshierarchie für Transport-Exceptions
...
:desc-image-011: Aufrufkette von der Service-Schnittstelle zum Anwendungskern
...
Letzteres wird im letzten `catch`-Block des obigen Code-Beispiels genutzt, da in diesem Fall keine gemäß <<KonzeptFehlerbehandlung>> aufgebaute Exception und somit weder Ausnahme-ID, UUID noch Fehlernachricht vorhanden sind.
...
:desc-image-012: Methoden der Klasse ExceptionMapper
...
Es können jedoch weitere Exceptions auftreten, die nicht gemäß <<KonzeptFehlerbehandlung>> aufgebaut sind.
...
:desc-listing-MappingExceptions: Beispielhaftes Mapping von Exceptions auf Ausnahme-IDs
...
=== Kompatibilität zu weiteren IAM-Systemen

Aufgrund der Umstellungen auf ein neues IAM-System und der Umstieg auf eine andere Schnittstellentechnologie (siehe <<KonzeptREST>>) kommt es übergangsweise zum Einsatz von unterschiedlichen Schnittstellentechnologien.
Um die Kompatibilität zu diesen Systemen zu gewährleisten, wurde der `AufrufKontextVerwalter` um das Attribut `Bearer Token` erweitert.
...
Bei der Verarbeitung eines HTTP-Request wird das `Bearer Token` aus dem Request gelesen und in dem AufrufKontextVerwalter gesetzt.

:desc-listing-handlerequest: Auslesen und Setzen des Bearer Tokens

[id="listing-handlerequest",reftext="{listing-caption} {counter:listings}"]
.{desc-listing-handlerequest}
[source,java]
----
@Override
...
In der Methode `prepareConnection` aus der Klasse `TimeoutWiederholungHttpInvokerRequestExecutor` wird der HTTP-Header um das `Bearer Token` erweitert.

:desc-listing-prepareconnection: Setzen des Bearer Tokens in HTTP-Header

[id="listing-prepareconnection",reftext="{listing-caption} {counter:listings}"]
.{desc-listing-prepareconnection}
[source,java]
----
@Override
...
Die Parameter für Timeout und Wiederholungen sind Bestandteil der betrieblichen Konfiguration (s. <<KonzeptKonfiguration>>).
