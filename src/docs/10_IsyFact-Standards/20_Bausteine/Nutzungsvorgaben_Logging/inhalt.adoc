...
Um die Korrelations-ID automatisch bei jedem Request zu setzen, muss in der `web.xml` der Servlet-Filter `HttpHeaderNestedDiagnosticContextFilter` für die gewünschten URLs konfiguriert werden.
Der Servlet-Filter liest automatisch bei jedem Request die Korrelations-ID aus dem Http-Header aus und setzt sie in den MDC.
Ist der Header nicht gesetzt, wird eine neue Korrelations-ID generiert.

// tag::important[]
IMPORTANT: Der `mdcFilter` sollte vor der `springSecurityFilterChain` definiert werden, da diese die gesetzte Korrelations-ID ausließt.
