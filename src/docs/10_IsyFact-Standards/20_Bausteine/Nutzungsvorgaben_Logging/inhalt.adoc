...
Es kann die maximale Länge eines Logeintrags in Bytes festgelegt werden. Sollte ein Logeintrag länger als dieser Wert sein,
so wird der Logeintrag gekürzt (siehe <<KonzeptLogging>>). Wird kein Wert angegeben, so beträgt der Standardwert 32000 Bytes. Wird der Wert auf 0
festgelegt, so gibt es keine Begrenzung in der Länge des Logeintrags. Die Beschränkung der Länge gilt nur für Logeinträge ab INFO-Level.
...
3.  **Größe des Logeintrags beschränken**: Logeinträge größer 32 KByte führen zu Fehlern bei der weiteren Verarbeitung.
Daher sollte darauf geachtet werden, dass Logeinträge die maximale Größe von 32 KByte nicht überschreiten. Zudem sollte die
maximale Länge von Logeinträgen in der Konfigurationsdatei `logback.xml` definiert sein: <<maximale-logeintragslaenge-festlegen>>.
