...
in Abstimmung mit dem Auftraggeber festzulegen siehe <<IsyFact-Namenskonventionen>>._ a|Name der Anwendung bzw. des Batches.
...
Es wird jedoch ein anderer Appender referenziert (<<listing-LoggingKonfigurationBatch>>) der ein größenbasiertes Rollieren umsetzt.

:desc-listing-LoggingKonfigurationBatch: Konfiguration des Loggings für Batches
[id="listing-LoggingKonfigurationBatch",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-LoggingKonfigurationBatch}
[source,xml]
----
<configuration scan="true" scanPeriod="1 minutes">
<!-- Pfad der Logdatei, ohne Endung -->
