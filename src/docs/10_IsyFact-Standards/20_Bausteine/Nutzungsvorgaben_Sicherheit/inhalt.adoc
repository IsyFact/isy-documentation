...
// TODO Überarbeitung bis hier!

[[gui-absicherung]]
==== Absicherung der Flows

Ein Webflow (oder auch ausgewählte Zustände und Transitionen) können durch das Tag `<secured/>` abgesichert werden.
Im Parameter `attributes` stehen die für den Zugriff erforderlichen Rechte.
Beispiel:

:desc-listing-absichern-eines-flow: Absichern eines Flow / Zustands / Transition
