...
Dazu bietet der Baustein Sicherheit über die Schnittstelle `Berechtigungsmanager` entsprechende Funktionen bereit.
...
Für diese Fälle stellt der Bausteins Sicherheit keine Funktionalität bereit.
...
Es ist ein analoger Ablauf wie beim Start von Batches (s. <<batch-autorisierung>>) umzusetzen.

[[aufrufen-von-nachbarsystemen]]
== Aufrufen von Nachbarsystemen

So wie ein IT-System bei einem Aufruf erwartet, einen gültigen, vollständigen Aufrufkontext vorzufinden, erwartet dies auch ein Nachbarsystem, das aufgerufen wird.
...
AufrufKontextTo transportobjekt = (AufrufKontextTo) dozer.map(aufrufKontext, AufrufKontextTo.class);
...
* Es wird ein Dozer-Mapping verwendet, um das für den Aufruf zu verwendende Transportobjekt zu erstellen.
...
Dazu ist die folgende Spring-Konfiguration nötig.

:desc-listing-konfiguration-korrelations-id: Spring-Konfiguration zur Entgegennahme der Korrelations-ID
[id="listing-konfiguration-korrelations-id",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-konfiguration-korrelations-id}
[source, xml]
----
<beans>
  <bean id="stelltLoggingKontextBereitInterceptor"
      class="de.bund.bva.pliscommon.aufrufkontext.service.StelltLoggingKontextBereitInterceptor"/>
  <aop:config>
...
Hierzu kann von der Standard-Implementierung (`de.bund.bva.pliscommon.aufrufkontext.impl.AufrufKontextImpl`) geerbt und die neue, anwendungsspezifische Klasse um zusätzliche Attribute erweitert werden.
...
<beans>
    <bean id="aufrufKontextFactory"
    ...
    <property name="aufrufKontextKlasse"
        value="<fqcn.des.anwendungsspezifischen.aufrufkontext>" />
  </bean>
----
...
Der Baustein Sicherheit bietet über die Schnittstelle `SicherheitAdmin` die Möglichkeit, die Verfügbarkeit eines IAM-Systems kontinuierlich zu prüfen.
...
Diese Methode kann in den Watchdog des IT-Systems (s. <<KonzeptUeberwachungKonfiguration>>) eingebunden werden.
...
<beans>
    <bean id="aufrufKontextVerwalter" scope="thread"
        class="de.bund.bva.pliscommon.aufrufkontext.stub.AufrufKontextVerwalterStub">
        <aop:scoped-proxy/>
        <property name="rollen">
            <list>
                <value>FAX_DialogNutzerA</value>
                <value>FAX_DialogNutzerAlle</value>
            </list>
        </property>
        <property name="durchfuehrenderBenutzerKennung" value="<kennung>" />
        <property name="durchfuehrenderBenutzerPasswort" value="<passwort>" />
        <property name="durchfuehrendeBehoerde" value="<behoerde>" />

        <property name="aufrufKontextFactory" ref="aufrufKontextFactory" />

        <property name="festerAufrufKontext" value="true/false" />
    </bean>
</beans>
----

Neben den gezeigten Eigenschaften lassen sich auch alle weiteren Eigenschaften des Aufrufkontextes konfigurieren.
Mit der Eigenschaft `festerAufrufKontext` lässt sich darüber hinaus steuern, ob bei jeder Anfrage der selbe Aufrufkontext (`true`) oder jedes Mal eine neue Instanz mit den gleichen, konfigurierten Werten (`false`) zurückgegeben wird.
