...
Der Info-Endpoint liefert statische Informationen über die Anwendung, und ist im Baustein Überwachung aktiviert. Die
Informationen müssen von der Anwendung explizit bereitgestellt werden.
Z.B. könnten Informationen über den Build und Quellcode-Stand (GIT) der Anwendung geliefert werden. Durch die
...
Bei Bedarf können weitere statische Anwendungsinformationen von der Anwendung bereitgestellt werden.
...
isy.ueberwachung.nachbarsysteme.beispielnachbar.healthendpoint=http://example.com/actuator/health
