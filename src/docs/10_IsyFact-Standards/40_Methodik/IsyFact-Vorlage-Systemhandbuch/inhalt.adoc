...
Typischerweise werden eine Fachanwendung und ein zugehöriges Service Gateway gemeinsam beschrieben, aber z.B. nicht zwei Fachanwendungen die zu unterschiedlichen Fachverfahren gehören, auch wenn Sie sich gegenseitig aufrufen.
...
Nach der SSL-Terminierung durch den Loadbalancer vor der Informations- & Dienstezone gelangen die HTTP-Requests zu einem Apache Web Server in der Informations- & Dienstezone.

Der Apache Server muss konfiguriert werden, um die Requests an den Tomcat der XXX-Fachanwendung in der Logik- und Verarbeitungszone weiterzuleiten.
...
Beispiel: +
`http://isy.local.vm:50001/xxx-ga`
...
2+^|*XXX-SGW*
|/var/log/isy-xxx-sgw/debug_log_HOSTNAME_xxx-sgw |Debug-Log des XXX Fachanwendung
...
|ID |Number(10, 0) |Technische ID der Person ^.^a|image::checkmark.png[align = center, width=40%]|
...
|GRUPPE_ID |Number (10, 0) |Verweis auf die Gruppe, zu der die Person gehört (siehe Tabelle GRUPPE) | ^.^a|image::checkmark.png[align = center, width=40%]
|===
...
NACHNACHME |
|… | | ^.^a|image::checkmark.png[align = center, width=40%]
||
|===
