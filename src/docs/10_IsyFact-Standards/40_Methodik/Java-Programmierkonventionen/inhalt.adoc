...
project-root/
  ...
  |- (.gitattributes)
  |- .gitignore
  |- CHANGELOG.md <!-- Dokumentation von Änderungen, z.B. via Eintrag einer Ticketnummer und kurzer Beschreibung -->
  |- (LICENSE.md)
  |- pom.xml
  |- README.md
----
...
Die einzelnen Quellcode-Dateien werden in einer Package-Struktur <<package-namen>> abgelegt.
...
Schnittstellen-Projekte bestehen in der Regel aus zwei Modulen. Ein Modul beinhaltet die angebotenen Interfaces, welche die
die nutzende Anwendung verwenden kann, um die angebotene Funktionalität nutzen zu können.
Zusätzlich können Schnittstellen-Objekte in diesem Modul vorhanden sein, welche zwischen den Schnittstellen als Transportobjekte
oder Exceptions eingesetzt werden können.
Das andere Modul ist die tatsächliche Implementierung der Anwendung auf der Seite des Schnittstellen-Anbieters.

Im Folgenden wird nur für das erste Modul die Struktur beschrieben, da für die tatsächliche Implementierung die
Beschreibung der Struktur, die der <<Anwendungen>> ist.

Schnittstellen-Module besitzen unter dem Wurzel-Verzeichnis (project-root) nur ein Unterverzeichnis: `src/main/java/`.
Darunter wird die Struktur konform zu <<package-namen>> angelegt mit der zusätzlichen Angabe aus dem Konzept IsyFact-Versionierung <<versionierung-von-schnittstellen>>

:desc-listing-Projektstruktur-Schnittstellen: Beispiel für eine Projektstruktur bei Schnittstellen-Modulen
...
project-root/
  |- src/main/java/ <!-- Hier befindet sich der Quellcode -->
  |           |- <package.schnittstellen-version>/
  |           |- MeinRemoteBean.java <!-- interface -->
  |           |- MeinTransportObjektTo.java <!-- ggf. auch weiteren Packeten/Verzeichnissen -->
  |           |- MeineToException.java <!-- ggf. auch weiteren Packeten/Verzeichnissen -->
----

[[batch]]
== Batch

Beim Batch-Modul besteht das Wurzel-Verzeichnis (project-root) aus dem einzelnen Unterverzeichnis `src/main/resources/`.

:desc-listing-Projektstruktur-Batch: Beispiel für eine Projektstruktur bei Batch-Modulen
[id="listing-Projektstruktur-Batch",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-Projektstruktur-Batch}
[source, xml]
----
project-root/
  |- src/main/resources/ <!-- Hier sind Ausführungsdateien (Shell-Skripte) und Konfigurationdateien,
                                wie z.B. <verfahren>.properties -->
  |           |- bin/
  |           |- meinBatch.sh
  |           |- meinBatch.bat
  |
  |- (.gitattributes) <!-- Falls das Modul kein Untermodul in einem bereits vorhanden git-Verzeichnis ist, kann diese Datei beinhaltet sein. Gleiches gilt für die folgenden optionalen Dateien. -->
  |- (.gitignore) <!-- OPTIONAL -->
...
Ein Deployment-Modul besteht im Wurzel-Verzeichnis (project-root) aus dem einzelnen Unterverzeichnis `src/main/resources/`.

:desc-listing-Projektstruktur-Deployment: Beispiel für eine Projektstruktur bei Deployment-Modulen
...
project-root/
  ...
  |           |- SRPMS/
  |
  |- build.xml <!-- Beschreibt die Vorgehensweise der Paketzusammenstellung und -erstellung -->
  |- (.gitattributes) <!-- Falls das Modul kein Untermodul in einem bereits vorhanden git-Verzeichnis ist, kann diese Datei beinhaltet sein. Gleiches gilt für die folgenden optionalen Dateien. -->
  |- (.gitignore) <!-- OPTIONAL -->
  |- (CHANGELOG.md) <!-- OPTIONAL -->
  |- (LICENSE.md) <!-- OPTIONAL -->
...
Ein Dokumentations-Modul besteht im Wurzel-Verzeichnis (project-root) aus dem einzelnen Unterverzeichnis `src/asciidoc/`
oder `src/adoc/`.
Unter dem `asciidoc` Verzeichnis ist es ebenfalls möglich eine eigene Ordnerstruktur anzulegen und
dort die Inhalte in `.adoc` Dateien zu legen. Bei mehreren Unterordnern empfiehlt sich eine `changelog.adoc`-Datei je Unterordner
anzulegen und diese im `project-root` in einer `changelog.adoc` für die gesamte Dokumentation zusammenzuführen.

:desc-listing-Projektstruktur-Dokumentation: Beispiel für eine Projektstruktur bei Dokumentations-Modulen
