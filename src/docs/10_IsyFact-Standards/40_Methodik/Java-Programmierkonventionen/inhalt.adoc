...
project-root/
  |- src/main/resources <!-- Hier sind Ausführungsdateien (Shell-Skripte) und Konfigurationdateien,
		...
		|- bin/
		    ...
		    |- meinBatch.bat
  |
  |- (.gitattributes) <!-- Falls das Modul kein Untermodul in einem bereits vorhanden
				git-Verzeichnis ist, kann diese Datei beinhaltet sein.
				Gleiches gilt für die folgenden optionalen Dateien. -->
  |- (.gitignore) <!-- OPTIONAL -->
