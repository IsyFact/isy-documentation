...
In diesem Abschnitt wird gezeigt, wie Sie den IsyFact Projektgenerator in der Spring Tool Suite (STS) installieren und Ihre Entwicklungsumgebung auf die Entwicklung
einer neuen Anwendung vorbereiten. Mit dem Projektgenerator werden Sie später die Hello-World-Anwendung erzeugen.

Für den IsyFact Projektgenerator müssen keine Bibliotheken manuell installiert werden. Benötigte Bibliotheken werden während des Build-Vorgangs automatisch aus Maven-Central geladen.
