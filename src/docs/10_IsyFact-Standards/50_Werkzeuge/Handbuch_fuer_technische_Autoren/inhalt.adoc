...
:desc-listing-admonition: Definition eines Admonition Blocks (als Block)
[id="listing-admonition",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-admonition}
[source,asciidoc]
----
[BLOCKTYP]
====
...
Die syntaktisch schlankere Variante funktioniert nur mit den vordefinierten Blöcken (NOTE, TIP, IMPORTANT, WARNING, CAUTION).

:desc-listing-admonition: Definition eines Admonition Blocks (inline)
[id="listing-admonition",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-admonition}
[source,asciidoc]
----
...
WARNING: Die Leerzeile zwischen dem Block und dem Ende des Tags ist wichtig, damit das Zusammenziehen der Inhalte korrekt funktioniert.
