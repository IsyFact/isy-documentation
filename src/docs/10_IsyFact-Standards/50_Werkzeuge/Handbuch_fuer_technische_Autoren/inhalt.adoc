...
In diesem Kapitel wird gezeigt, wie hierfür zunächst das Gesamtprojekt aus dem Git-Repository heruntergeladen wird, um anschließend die Dokument-Vorlage für das eigene IsyFact-Dokument vorzubereiten.
...
----
$> git clone https://github.com/IsyFact/isy-documentation
----

[[den-richtigen-standort-bestimmen]]
=== Den richtigen Standort bestimmen

Beim Herunterladen des Git-Repositories wird an der betroffenen Stelle das Verzeichnis `isy-documentation` angelegt.
Innerhalb von `isy-documentation` liegt das Verzeichnis `/src/docs`, das die Dokumente beinhaltet.
...
image::verzeichnis1.png[align="center",width=20%]
...
Die Unterordner der beiden Verzeichnisse `10_IsyFact_Standards` und `20_IsyFact_Erweiterungen` gliedern sich nach den Säulen der IsyFact.
...
image::verzeichnis2.png[align="center",width=30%]

Unterhalb der Säulen werden in der Regel die Verzeichnisse der IsyFact-Dokumente angelegt.
In der folgenden Abbildung sehen Sie beispielsweise, dass das "Handbuch für technische Autoren" unterhalb der Säule _50_Werkzeuge_ angelegt wurde.
...
image::verzeichnis3.png[align="center",width=40%]
...
Um Sie (als technischen Autor) bei der Erstellung des neuen IsyFact-Dokuments zu unterstützen, wird eine Dokomentvorlage bereitgestellt.

Die zum Build der Dokumentation notwendigen allgemeinen Anteile (globale Einstellungen, Themes, etc.) und die Dokumentvorlage für neue Dokumente befinden sich im gesonderten Maven-Artefakt `isy-dokumentvorlagen`.
Beim Build werden die Inhalte von `isy-dokumentvorlagen` nach `/target` unterhalb von `isy-documentation` entpackt.
Um an die Dokumentvorlage zu gelangen, ohne den kompletten Build zu starten, wird `isy-dokumentvorlagen` mit dem Kommando

----
$> mvn generate-sources
----

unter `/target/isy-dokumentvorlagen` entpackt.
Die Dokumentvorlage befindet sich dann unter `/target/isy-dokumentvorlagen/docs/Dokument-Vorlage`.

In diesem Verzeichnis befinden sich die Musterinhalte, die sie als Grundlage für Ihre neues Dokument  verwenden können.
Sobald Sie also das Zielverzeichnis für Ihr IsyFact-Dokument angelegt haben, kopieren Sie darin die Musterdateien aus dem Ordner `/target/isy-dokumentvorlagen/docs/Dokument-Vorlage`.
Erstellen Sie danach im Zielverzeichnis das Unterverzeichnis `images` für die Abbildungen Ihres IsyFact-Dokuments.
