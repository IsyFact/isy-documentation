...
1. Zentrale Dokumentation (isy-documentation)

2. Bausteindokumentation

3. Dokumentvorlagen (isy-dokumentvorlagen)
...
Damit jedoch auch alle Änderungen eines Projekts (Menge aller Komponenten/Dokumente) zentral in einem Dokument zusammengefasst werden, wird eine weitere Changelog-Datei im root-Verzeichnis des Projekts angelegt.
Diese Changelog-Datei, im Folgenden Projektchangelog genannt, bindet die einzelnen Changelog-Datei automatisch und nach Release gruppiert ein.
...
:desc-image-architektur_changelogs: Zusammenführung der einzelnen Komponentenchangelogs zu einem Projektchangelog
[id="image-architektur_changelogs",reftext="{figure-caption} {counter:figures}"]
.{desc-image-architektur_changelogs}
image::architektur_changelog.png[align="center",width=80%,pdfwidth=80%]
...
:desc-image-ablage_komponentenchangelog_dokument: Ablageort des Komponentenchangelog für ein Dokument
[id="image-ablage_komponentenchangelog_dokument",reftext="{figure-caption} {counter:figures}"]
.{desc-image-ablage_komponentenchangelog_dokument}
image::ablage_komponentenchangelog_dokument.png[align="center",width=60%,pdfwidth=60%]

Bei Komponenten wird diese Datei im root-Verzeichnis der Komponente abgelegt:

:desc-image-ablage_komponentenchangelog_komponente: Ablageort des Komponentenchangelog für eine Komponente
[id="image-ablage_komponentenchangelog_komponente",reftext="{figure-caption} {counter:figures}"]
.{desc-image-ablage_komponentenchangelog_komponente}
image::ablage_komponentenchangelog_komponente.png[align="center",width=60%,pdfwidth=60%]

Die Datei wird zunächst ohne Inhalt angelegt.
...
:desc-image-ablage_projektchangelog: Ablageort des zentralen Projektchangelogs
[id="image-ablage_projektchangelog",reftext="{figure-caption} {counter:figures}"]
.{desc-image-ablage_projektchangelog}
image::ablage_projektchangelog.png[align="center",width=60%,pdfwidth=60%]

Initial wird die Datei mit der Überschrift "Changelog" befüllt:

:desc-listing-projektchangelog_ueberschrift: Projektchangelog Inhalt
[id="listing-projektchangelog_ueberschrift",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-projektchangelog_ueberschrift}
[source,adoc]
----
= Changelog
----
...
:desc-listing-komponentenchangelog_release: Erstellung Release-Tag im Komponentenchangelog
[id="listing-komponentenchangelog_release",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-komponentenchangelog_release}
[source,adoc]
----
// tag::release-2.1.0[]
...
Jeder Release-Tag enthält die Versionsnummer des Releases.
...
:desc-listing-projektchangelog_release: Erstellung Release
[id="listing-projektchangelog_release",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-projektchangelog_release}
[source,adoc]
----
...
:desc-listing-projektchangelog_einbindung_komponentenchangelog: Einbindung der Komponentenchangelogs
[id="listing-projektchangelog_einbindung_komponentenchangelog",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-projektchangelog_einbindung_komponentenchangelog}
[source,adoc]
----
== Änderungen IsyFact 2.1.0
...
:desc-listing-projektchangelog_einbindung_releases: Einbindung mehrerer Releases
[id="listing-projektchangelog_einbindung_releases",reftext="{listing-caption} {counter:listings }"]
.{desc-listing-projektchangelog_einbindung_releases}
[source,adoc]
----
== Änderungen IsyFact 2.1.0
...
2. Migrationsanleitung von einer älteren Version der IsyFact auf die aktuelle Version
