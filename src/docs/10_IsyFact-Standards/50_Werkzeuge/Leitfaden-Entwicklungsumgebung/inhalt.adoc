...
Es erscheint ein neues Fenster, bei dem Sie im ersten Eingabefeld "Ubuntu Gnome 17.04 (64bit)" eingeben. Setzen Sie die Version auf Ubuntu (64-bit).
...
Klicken Sie auf Weiter. Hierdurch wird VirtualBox das VM-Verzeichnis zur virtuellen Maschine erstellen. Das Verzeichnis trägt den gleichen Namen wie die virtuelle Maschine,
nämlich "Ubuntu Gnome 17.04 (64bit)".
...
in das Verzeichnis der virtuellen Maschine speichert. Ãœber die Option "Festplatte erzeugen" würden Sie diese Datei normalerweise ganz neu anlegen,
...
Anleitung eine Abkürzung. Die Webseite http://www.osboxes.org bietet virtuelle Festplatten mit vorinstallierten Betriebssystemen an. Unter http://www.osboxes.org/ubuntu-gnome
wird beispielsweise eine Festplatte angeboten, auf der Ubuntu mit der Gnome-Oberfläche vorinstalliert ist. Diese Datei können Sie in komprimierter Form auch direkt über folgende
URL herunterladen:

 https://drive.google.com/uc?export=download&confirm=ZacZ&id=0B_HAFnYs6Ur-VWZ5TTVSd1dScEE

Nach dem Herunterladen befindet sich eine Datei mit dem Namen `Ubuntu_Gnome_17.04-VB-64bit.7z` in Ihrem Download-Verzeichnis. Entpacken Sie diese Datei in das
VM-Verzeichnis "Ubuntu Gnome 17.04 (64bit)". Danach wechseln Sie wieder in das VirtualBox Programm, um die Festplatte einzubinden.
...
Klicken Sie links oben auf "Applications" und dann unten auf "Activities Overview", um eine Ãœbersicht über die Applikationen zu bekommen.
...
Abschließend passen Sie die visuelle Oberfläche Ihres Ubuntu Gnome Betriebssystems an, indem Sie im Hauptmenü auf "Applications" - "Utilities" - "Tweak Tools" klicken.
Im erscheinenden Fenster, klicken Sie links auf Desktop.

Über ein Häkchen, jeweils in der Checkbox "Home" und "Trash", werden zwei neue Symbole auf dem Desktop für das Home-Verzeichnis und für den Mülleimer erstellt.

Über Location Background und Lock Screen Mode wechseln Sie die Hintergrundbilder.

:desc-image-leitfaden_entwicklungsumgebung_vb_ubuntu_tweak_01: Hintergrundbild einrichten
[id="image-leitfaden_entwicklungsumgebung_vb_ubuntu_tweak_01",reftext="{figure-caption} {counter:figures}"]
.{desc-image-leitfaden_entwicklungsumgebung_vb_ubuntu_tweak_01}
image::leitfaden_entwicklungsumgebung_vb_ubuntu_tweak_01.png[align="center"]

Top Bar - Show Date und Show Seconds

:desc-image-leitfaden_entwicklungsumgebung_vb_ubuntu_tweak_02: Uhrzeitanzeige einrichten
[id="image-leitfaden_entwicklungsumgebung_vb_ubuntu_tweak_02",reftext="{figure-caption} {counter:figures}"]
.{desc-image-leitfaden_entwicklungsumgebung_vb_ubuntu_tweak_02}
image::leitfaden_entwicklungsumgebung_vb_ubuntu_tweak_02.png[align="center"]
...
Um STS darüber hinaus über eine Desktop-Verknüpfung starten zu können, muss im Verzeichnis `.local/share/applications` Ihres Home-Verzeichnisses eine Datei des
Typs `.desktop` abgelegt werden. Diese können Sie bequem mit dem Programm `gnome-panel` erzeugen. Installieren Sie `gnome-panel` mit folgender Anweisung:
...
:desc-image-leitfaden_entwicklungsumgebung_werkzeuge_sts_icon: Spring Tool Suite (STS) Einrichtung des Namens
[id="image-leitfaden_entwicklungsumgebung_werkzeuge_sts_icon",reftext="{figure-caption} {counter:figures}"]
.{desc-image-leitfaden_entwicklungsumgebung_werkzeuge_sts_icon}
image::leitfaden_entwicklungsumgebung_werkzeuge_sts_icon.png[align="center",width=60%,pdfwidth=60%]
...
Unterprojekte aufgeteilt. Ãœberspannt werden die Unterprojekte durch das Maven-Projekt `isyfact-base`.
