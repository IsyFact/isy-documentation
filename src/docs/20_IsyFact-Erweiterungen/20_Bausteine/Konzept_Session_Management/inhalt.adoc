...
`IsySessionConfiguration` ist die zentrale Klasse der Komponente.
...
Es wird über die Konfiguration definiert, welche Zugriffe mit einem Leseschloss bzw. Schreibschloss versehen werden sollten. Somit wird es verhindert, das es zur Inkonsistenz in der Sessioninformation kommen kann.
