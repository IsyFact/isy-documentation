...
- Festlegung eines minimalen Sets an Attributen, die über das JWT ausgetauscht werden sollen.
- Bereitstellung von Konfigurationen für den Keycloak Server
...
|Enthält die Implementierung des `AccessManagers` aus _isy-sicherheit_.
Der Baustein übernimmt die Authentifizierung von REST-Services sowie von HttpInvoker-Services über den Aufrufkontext.
...
Für Behörden oder Institute, die zunächst auf Keycloak umstellen wollen und anschließend die Authentifizierung für die Externen Clients umstellen möchten, gibt es eine Zwischenlösung, bei welcher vorerst das Service-Gateway die Authentifizierung der Clients weiterhin übernimmt (vgl. <<image-auth-sgw-legacy>>).
...
|Isy Sicherheit
|Bietet eine Konfiguration für ein Rollen & Rechte Mapping an, sowie verschiedene Features zur Absicherung von IsyFact Bausteinen.
...
Ein direkter Zugriff auf da Access-Token in einer Anwendung ist nicht notwendig, stattdessen werden die notwendigen Claims des Tokens mit Hilfe eines Filters in ein `Authorization` Objekt gewandelt.
