...
image::aufbauisysession.png[align="center"]

RedisClusterKonfiguration ist die zentrale Klasse der Komponente.
Sie initialisiert Spring Session mittels Java-Konfiguration gemäß dem offiziellen HttpSession Guide für Redis.
...
|Einflussfaktoren |Risiko: Sessions werden gegenseitig überschrieben und Session-Daten gehen verloren.
|Annahmen a|
...
(Stichwort: watch) umgesetzt werden, diese wird aktuell aber nicht durch Spring-Data unterstützt 
...
image::clusterredis.png[align="center"]

Die vollständige technische Infrastruktur des Clusters wird im <<Nutzungsvorgaben-Redis>> beschrieben.
...
|Fragestellung |Welche Sicherheitsmaßnahmen werden innerhalb des Session-Speichers und beim Zugriff auf den Speicher umgesetzt?
|Einflussfaktoren a|
...
|Annahmen |Redis wird in einer sicheren Umgebung betrieben.
|Betrachtete Alternativen a|
...
|Fragestellung |Sollen die Session-Daten der verschiedenen Anwendungen innerhalb des Redis-Clusters voneinander logisch getrennt werden?
|Einflussfaktoren |
|Annahmen |Session-IDs sind eindeutig, so dass keine Konflikte auftreten können, wenn mehrere Anwendungen die gleiche Datenbank verwenden.
|Betrachtete Alternativen a|
...
|Fragestellung |Sollen die Daten auf der Festplatte persistiert werden, oder ausschließlich „in-memory“ verwaltet werden?
|Einflussfaktoren |
|Annahmen |
|Betrachtete Alternativen a|
...
|Fragestellung |In welcher Zone soll Redis betrieben werden?
|Einflussfaktoren a|
