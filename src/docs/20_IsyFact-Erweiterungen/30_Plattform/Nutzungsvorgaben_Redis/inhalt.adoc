...
image::rediscluster.png[align="center"]
//fig1
...
Die Kommunikation zwischen Webanwendung zu Redis und Redis zu Redis erfolgt mittels RESP (REdis Serialization Protocol) über TCP-Verbindungen.
...
=== redis_<server/master>.conf anpassen
...
Wichtig: Hierbei müssen IPs angegeben werden, keine Hostnamen!

Die nachfolgende Frage „Can I set the above configuration?” muss mit “yes” bestätigt werden.
...
image::createcluster.png[align="center"]
...
.  Neuen Server analog zu Kapitel <<installation-und-konfiguration>>, Schritte 1-3 einrichten
.  Alle Instanzen stoppen
.  Alle `nodes_<master/slave>.conf` Dateien löschen
.  Alle Instanzen starten
.  Cluster-Konfigurieren analog zu Schritt 6 aus Kapitel 3
...
siehe: `https://github.com/antirez/redis/issues/2186`
...
Um Redis in Docker verwenden zu können, muss der „host networking mode“ von Docker verwendet werden.
